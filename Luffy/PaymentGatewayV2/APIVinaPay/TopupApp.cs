...
namespace APIVinaPay
{
    internal class TopupApp
    {
        ...
        private string CallbackUrlV2 = "http://149.28.151.96:1596/CardCallbackV2.ashx";
        private string CallbackUrl = "http://149.28.151.96:1596/CardCallback.ashx";
        private const string apiKey = "api46@";
        private const string username = "api46@gmail.com";
        private const int ClientID = 18;
        ...
        {
            var token = "";
            var dataCache = DataCaching.GetCache<string>("VinaPaysTokenV2025");
            if (dataCache != null)
            {
            ...
            else
            {
                ...
                if (!string.IsNullOrEmpty(response))
                {
                    ...
                    DataCaching.SetCache("VinaPaysTokenV2025", token, 3600*3);
                }
            }
        ...
        public async Task<string> VinaPayTopupCallBack(string transactionId, string telco, string partnerCode, string providerCode, string serial, string pin, int amount)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(response))
                {
                    ...
                    {
                        TelegramNotify.SendWarning("1690000254", "Provider ship bị lỗi");
                        TelegramNotify.SendTeleZoro(-659770536, "Server Error");
                        return "-1|0";
                    }
                    ...
                    switch (res.error_code)
                    {
                        ...
                        case 6:
                            ...
                            TelegramNotify.SendWarning("1690000254", "Provider ship bị lỗi");
                            TelegramNotify.SendTeleZoro(-659770536, "Hệ thống không đủ tài nguyên hoặc đang bảo trì");
                            return "-1|0";
                        default:
                            DataRequest.UpdateTopupCard(topup.Id, 0, -1, string.Empty, string.Empty);
                            return "-1|0";
