...
namespace Libs.Report
{
    ...
    public class CardAPILog
    {
        ...
        public CardAPILog Get(int transactionID)
        {
            DBHelper db = new DBHelper(Configs.VPGLogConnectionStrings);
            return db.GetInstanceSP<CardAPILog>("sp_CardAPILog_Select"
                , new SqlParameter("@TransactionID", transactionID));
        }
        public CardAPILog GetBySeri(string seri)
        {
            DBHelper db = new DBHelper(Configs.VPGLogConnectionStrings);
            return db.GetInstanceSP<CardAPILog>("sp_CardAPILog_SelectBySeri"
        ...
        public List<CardAPILog> GetTableListV2(int top, string PartnerCodes, DateTime creatTime, DateTime endTime, int? status, string cardType, string provider)
        {
            DBHelper db = new DBHelper(Configs.VPGLogReportConnectionStrings);
        ...
        public List<CardAPILog> ReportDoiSoatDaily(string partnerCodes, string provider, string cardType, DateTime beginTime, DateTime endTime, int TypeSelect)
        {
            DBHelper db = new DBHelper(Configs.VPGLogReportConnectionStrings);
            SqlParameter[] pars = new SqlParameter[6];
            pars[0] = string.IsNullOrEmpty(partnerCodes) ? new SqlParameter("@PartnerCodes", DBNull.Value) : new SqlParameter("@PartnerCodes", partnerCodes);
            ...
            pars[4] = new SqlParameter("@EndTime", endTime);
            pars[5] = new SqlParameter("@TypeSelect", TypeSelect);
            var data = db.GetListSP<CardAPILog>("sp_CardAPILog_ReportDSDaily", pars);
