...
namespace Libs.CardTelco.Khoai
{
    public class KhoaiCard : ICardTelcoHandler
    {

        protected string providerName = string.Empty;
        string serviceUrl = "http://127.0.0.1:1597/";
        ...
        public APIResponse UseCard(APITransaction transaction)
        {
            ...
            try
            {
                ...
                if (!Utils.GlobalHelper.CheckCardCode(request.CardType, request.CardCode))
                {
                    ...
                    //if (request.CardCode.Length > 10 && request.CardCode.Length < 17)
                    //    TelegramNotify.SendWarning("1690000254", $"Mã seri {request.CardSerial}  thẻ {request.CardCode}  mạng {request.CardType} sai định dạng thẻ");
...

                {
                    ...
                    //if (request.CardSerial.Length > 10 && request.CardSerial.Length < 17)
                    //    TelegramNotify.SendWarning("1690000254", $"Mã seri {request.CardSerial} thẻ {request.CardCode} mạng {request.CardType} sai định dạng seri ");
                ...
                if (_CardAPILog.ReturnValue < 0)
                {
                    ...
                    {
                        NLogLogger.Info(new string[] { "KhoaiCard", transaction.TransactionID.ToString(), transaction.PartnerCode, "TopupRequest", request.CardSerial, request.CardCode, "Error Insert Data" });
                ...
                NLogLogger.Info(new string[] { "KhoaiCard", transaction.TransactionID.ToString(), "TopupRequest", serializer.Serialize(topupRequest) });
                TopupResponse cardResult = ProcessCard(topupRequest);
                NLogLogger.Info(new string[] { "KhoaiCard", transaction.TransactionID.ToString(), "TopupResponse", serializer.Serialize(cardResult) });
                ...
                else
                {
                    switch (cardResult.code)
                    {
                        ...
                        case (int)ResponseCode.TransactionTimeout:
                            ...
                            _CardAPILog.Status = _APIResponse.ResponseCode;
                            _APIResponse.ResponseContent = "khoai";
                            break;
                    ...
                    _CardAPILog.Description = cardResult.code.ToString();
                }
            }
            catch (Exception ex)
            {
                ...
                {
                    case 1:
                        NLogLogger.Info(new string[] { "KhoaiCard", transaction.TransactionID.ToString(), "Error", "Step1", ex.Message.Replace("\n", " ") });
                        _APIResponse = new APIResponse((int)ResponseCode.RequestContentInvalid);
                        break;
                    case 2:
                        NLogLogger.Info(new string[] { "KhoaiCard", transaction.TransactionID.ToString(), "Error", "Step2", ex.Message.Replace("\n", " ") });
                        _APIResponse = new APIResponse((int)ResponseCode.SystemError);
                        break;
                    case 3:
                        NLogLogger.Info(new string[] { "KhoaiCard", transaction.TransactionID.ToString(), "Error", "UseCard", ex.Message.Replace("\n", " ") });
                        _CardAPILog.Description = "Error - UseCard " + ex.Message.Replace("\n", " ");
                        _APIResponse = new APIResponse((int)ResponseCode.TransactionSuspicious);
                        break;
                    case 4:
                        NLogLogger.Info(new string[] { "KhoaiCard", transaction.TransactionID.ToString(), "Error", "Step4", ex.Message.Replace("\n", " ") });
            ...
            {
                NLogLogger.Info(new string[] { "CardAPILog", serializer.Serialize(_CardAPILog) });
        ...
        {
            JavaScriptSerializer serializer = new JavaScriptSerializer();
            //NLogLogger.Info(new string[] { "KhoaiCard", "TopupRequest", serializer.Serialize(topupRequest) });
            ...
            //NLogLogger.Info(new string[] { "KhoaiCard", "TopupResponse", serviceResponse });
            ...
            try
            {
                NLogLogger.Info(new string[] { "APIKhoai", "Request", topupRequest.transactionId.ToString(), serviceResponse });
                return serializer.Deserialize<TopupResponse>(serviceResponse);
            }
            catch (Exception e)
            {
                NLogLogger.Info(new string[] { "APIKhoai", "Request", topupRequest.transactionId.ToString(), e.Message });
                return new TopupResponse()
                {
                    code = (int)ResponseCode.TransactionTimeout,
