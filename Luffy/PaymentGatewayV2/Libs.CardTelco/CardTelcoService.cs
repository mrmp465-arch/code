...
namespace Libs.CardTelco
{
    ...
    public class CardTelcoService : IServiceHandler
    {
        ...
        private APIResponse UseCard(APITransaction transaction)
        {
            ...
            if (request.CardType.ToLower() != "viettel" && request.CardType.ToLower() != "vnp" && request.CardType.ToLower() != "vms" && request.CardType.ToLower() != "zing" && request.CardType.ToLower() != "gate" && request.CardType.ToLower() != "vnm")

            {
                return new APIResponse((int)ResponseCode.CardTypeInvalid);
            }
            if (request.CardType.ToLower() == "zing" && (request.Amount == 30000 || request.Amount == 300000))
            ...
            //if (transaction.PartnerCode == "panda")
            //{
            //    var oldproviderCode = providerCode;
            ...
            //        providerCode = oldproviderCode;
            //}
            if ((transaction.PartnerCode == "gta2") || (transaction.PartnerCode == "gta3") || (transaction.PartnerCode == "portal16") || (transaction.PartnerCode == "dream86"))
            {
                string oldproviderCode = providerCode;
                if (request.CardType.ToLower() == "viettel")
                {
                    providerCode = "alovtt";
                }
                ...
                if (new Providers().Get(providerCode).Status != 1)
                {
                    providerCode = oldproviderCode;
                }
            }
            if (request.CardSerial== "20000250512593" || request.CardCode.Contains("023229348"))
