...
namespace Libs.TopupPartner.M32
{
    public class M32Service : IBuyCardHandler
    {
        ...
        protected string ServiceUrl = "https://n3s172.ghostspam.com/api/buycard";

        
        protected string apiPass = "766816cf-13f1-462e-a8c7-3e23bedc75b7";
        ...
        public APIResponse downloadSoftpin(string requestId, string provider, int amount, int quantity, ref string providerResponse)
        {
            ...
            var signature = Encrypts.MD5(apiPass + requestId+ provider+ amount);
            var data = string.Format("api_key={0}&telco={1}&amount={2}&request_id={3}&signature={4}", apiPass, provider,amount, requestId ,signature );
            NLogLogger.Info(new string[] { "Bpay", "Request", requestId, data });
            var responseData = Task.Run(async () => await PostTask(ServiceUrl, data)).Result;
            NLogLogger.Info(new string[] { "Bpay", "Response", requestId, responseData });
        ...
        public APIResponse ConvertResultCode(ResultData result)
        {
            ...
            switch (result.status)
            {
                ...
                case 0:
                    ...
                    //        }
                    //        ); ;
                    var listCard = result.card_seri + "|" + result.card_code;
