    ...
    protected void btUpdate_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            if (_CardAPILog.Status == 1 || _CardAPILog.Status == 2)
            {
                ...
                if (oldStatus < 1)
                {
                    if(_CardAPILog.PartnerCode=="azt")
                    {
                        Action<string, long, string,string> send = UpdatePartnerBalance;
                        var asynSend = send.BeginInvoke(_CardAPILog.PartnerCode, Math.Min(Convert.ToInt64(_topupMobile3rdLog.Amount), _topupMobile3rdLog.AmountUser), _CardAPILog.CardType.ToLower(), _CardAPILog.CardSerial, null, null);
            ...
            {
                var amount = (int)Math.Min(_CardAPILog.Amount, _CardAPILog.AmountUser);
                if(amount==0)
                {
                    amount =(int) _CardAPILog.Amount;
                }    
                var datacb = new DataCallback()
                {
                    ...
                    Status = cbStatus,
                    Signature = Libs.Utils.Encrypts.MD5(_CardAPILog.RequestNo + cbStatus + amount + privateKey)
                };
