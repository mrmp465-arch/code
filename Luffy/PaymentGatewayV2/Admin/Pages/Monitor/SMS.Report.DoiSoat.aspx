<%@ Page Title="" Language="C#" MasterPageFile="~/Layout/Layout.master" AutoEventWireup="true" CodeFile="SMS.Report.DoiSoat.aspx.cs" Inherits="Pages_Monitor_SMS_Report_DoiSoat" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <section class="content">
                        ...
                        <table class="table table-bordered nowrap" id="TableResponsive1" style="margin-top: 20px; clear: both;" cellspacing="0" width="100%">
                            ...
                            <tbody>
                                <% long Total = 0;
                                    if (lstReportDoiSoat1 != null)
                                    {
                                        ...
                                        foreach (var item in lstReportDoiSoat1)
                                        {
                                            ...
                                            int count2 = 0;
                                            if (CardType != item.ReceiverNumber)
                                            {
                                                count2 = lstReportDoiSoat1.Count(e => e.ReceiverNumber == item.ReceiverNumber && e.PartnerId == item.PartnerId);
                                                CardType = item.ReceiverNumber;
                                            }  %>
                                <tr style="vertical-align: middle;">
                                    ...
                                    <%} %>
                                    <%if (count2 > 0)
                                        { %><td rowspan="<%=count2 %>"><%= item.ReceiverNumber %></td>
                                    <%} %>
                                    ...
                                    <td class="right"><%= Convert.ToInt64((item.ReturnValue*item.Amount)).ToString("N0") %></td>
                                </tr>
                                <% Total += item.ReturnValue * item.Amount;
                                        }
                                    } %>
                                <tr class="Total">
                                ...
                                <% long Total2 = 0;
                                    if (lstReportDoiSoat2 != null)
                                    {
                                        ...
                                        foreach (var item in lstReportDoiSoat2)
                                        {
                                            ...
                                            if (CardType != item.ReceiverNumber)
                                            {
                                                count2 = lstReportDoiSoat2.Count(e => e.ReceiverNumber == item.ReceiverNumber && e.Provider == item.Provider);
                                                CardType = item.ReceiverNumber;
                                            } %>
                                <tr>
      ...
      { %><td rowspan="<%=count2 %>"><%= item.ReceiverNumber %></td>
                                ...
                                <% Total2 += item.ReturnValue * item.Amount;
                                        }
                                    } %>
                                <tr class="Total">
                                    <td colspan="5" style="text-align: center;"><b>Total</b></td>
