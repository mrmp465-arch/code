...
namespace APITopupMobile
{
    ...
    public class TopupCallbackAppVTT : System.Web.Services.WebService
    {
        ...
        public string Callback(string transactionId, int status, int amount, string message, string logContent, string sign)
        {
            ...
            if (!string.IsNullOrEmpty(transactionId))
            {
                ...
                NLogLogger.Info(new string[] { "TopupCallbackAppVTT", "Request", transactionId, amount.ToString(), status.ToString() });
                ...
                if (topup3Rd != null)
                {
                    ...
                    //if (1 == 1)
                    {
                        var topupmobile = new TopupMobileLog().Get(Convert.ToInt64(topup3Rd.RequestNo));
                        ...
                        switch (status)
                        {
                            case (int)ResponseCode.TransactionSuspicious:
                                ...
                                if (topup3Rd.Status != 0)
                                {
                                    ...
                                    Task.Run(async () => await new MyViettelBiz().CallbackJson(cardAPILog.CallbackUrl, serializer.Serialize(data), cardAPILog.PartnerCode, topup3Rd.TransactionId, 2).ConfigureAwait(false));
                                }

                                topupmobile.Topup(1, amount == 0 ? topup3Rd.AmountUser : amount); // + So tien don hang
                                NLogLogger.Info(new string[] { "TopupCallbackAppVTT", cardAPILog.PartnerCode, "Request", "TransactionSuspicious", topupmobile.TransactionID.ToString(), amount.ToString() });
                                cardAPILog.Amount = 0;
                                break;
                            case (int)ResponseCode.TransactionSuccessfullNotConfirmYet:

                                var lastAmount = amount - topup3Rd.AmountUser;
                                if (topup3Rd.Status == -2)
                                    topupmobile.Topup(1, lastAmount, topup3Rd.BidRate); // Tinh toán lại mệnh giá thật
                                //else
                                //    topupmobile.Topup(1, amount, topup3Rd.BidRate);
                                ...
                                if (!string.IsNullOrEmpty(topupmobile.CallbackUrl))
                                {
                                    var data = new DataCallbackOrder()
                                    {
                                        Amount = amount,
                                        OrderId = topupmobile.TransactionID,
                                        Status = 1,
                                        CardSerial = topup3Rd.CardSerial,
                                        CardCode = topup3Rd.CardCode,
                                        BidRate = topupmobile.BidRate,
                                        UpdateTime = DateTime.Now,
                                        CreatTime = topup3Rd.CreateTime,
                                        Signature = string.Empty
                                    };
                                    Task.Run(async () => await new MyViettelBiz().CallbackJson(topupmobile.CallbackUrl, serializer.Serialize(data), topupmobile.UserName, topup3Rd.Id, 1).ConfigureAwait(false));
                            ...
                            case (int)ResponseCode.TransactionCancel:
                                if (topup3Rd.Status == -2) 
                                    topupmobile.Topup(-5, topup3Rd.AmountUser, topup3Rd.BidRate); //- So tien don hang
                                //Callback KH
                            ...
                            case (int)ResponseCode.TransactionSuccessful:
                                if (topup3Rd.Status != 0)
                                {
                                    NLogLogger.Info(new string[] { "TopupCallbackAppVTT", "Request", "Transaction Successful", transactionId, amount.ToString(), status.ToString() });
                                    topupmobile.Topup(1, amount, topup3Rd.BidRate); // + So tien don hang
                        ...
                        topup3Rd.LogContent = "Review " + status;
                        topup3Rd.Update();
            ...
            NLogLogger.Info(new string[] { "TopupCallbackAppVTT", "Response", transactionId, amount.ToString(), status.ToString() });
        ...
        //{
        //    NLogLogger.Info(new string[] { "TopupCallbackAppVTT", "Callback", "Request", code, url, postData });
        //    var uri = new Uri(url);
        //    try
        //    {
        ...
        //                NLogLogger.Info(new string[] { "TopupCallbackAppVTT", "Callback", "Response", code, url, postData });
        ...
        //    catch (Exception e)
        //    {
        //        NLogLogger.Info(new string[] { "TopupCallbackAppVTT", "Callback", e.Message });
        //        return string.Empty;
        //    }
