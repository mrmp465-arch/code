...
namespace Libs.API
{
    ...
    public class Providers
    {
        ...
        public string GetBuyCardCondition(string productCode)
        {
            DBHelper db = new DBHelper(Configs.VPGAPIReportConnectionStrings);
        ...
        public string GetBuyCardConditionCache(string productCode)
        {
            ...
            try
            {
                ...
                if (result == null)
                {
                    result = GetBuyCardCondition(productCode);
                    result = DataCaching.SetCache(KeyCache, result);
                }
                return result;
            }
        ...
        public string GetBankCashCondition(string productCode, string partnerCode)
        {
            DBHelper db = new DBHelper(Configs.VPGAPIReportConnectionStrings);
            SqlParameter[] pars = new SqlParameter[3];
            pars[0] = new SqlParameter("@ProviderCodeReturn", SqlDbType.VarChar, 50) { Direction = ParameterDirection.Output };
            pars[1] = new SqlParameter("@ProductCode", productCode);
            pars[2] = new SqlParameter("@PartnerCode", partnerCode);
            db.ExecuteNonQuerySP("sp_Providers_Select_BankCash_Condition", pars);
            var returnValue = pars[0].Value.ToString();
            return returnValue;
        }
