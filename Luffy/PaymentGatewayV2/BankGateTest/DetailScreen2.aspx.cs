...
{
    public partial class DetailScreen2 : System.Web.UI.Page
    {
        protected async void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (!string.IsNullOrEmpty(orderNo) && !string.IsNullOrEmpty(refcode) && !string.IsNullOrEmpty(chatid))
                {
                  ...
                  botToken: "123456:ABC...",
                    //chatId: "-1003089655183",
                    chatId: chatid,
                  url: "https://info.24happ.info/Pages/Detail.aspx?orderNo=" + orderNo,
                  billSelector: "#bill",
                  caption: refcode 
              );
                }
       ...
       string chatId,
       string url,
       string billSelector,   // ví dụ: "#bill", ".bill-container", "#ctl00_ContentPlaceHolder1_bill"
       string caption = null)
            {
                    ...
                    Args = new[]
      {
        ...
        "--user-data-dir=C:\\chrome-profile"
                ...
                using (var page = await browser.NewPageAsync())
                {
                    ...
                    await SendPhotoTelegramAsync(botToken, chatId, png, "bill.png", caption);
                }
            }

            private static async Task SendPhotoTelegramAsync(string botToken, string chatId, byte[] photo, string fileName, string caption)
            {
                var apiUrl = "https://api.telegram.org/bot8527601171:AAG_fbFEAqXGhQFfYmxh3oevGM8OWZfRpK0/sendPhoto";
