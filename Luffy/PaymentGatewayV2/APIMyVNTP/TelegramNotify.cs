            ...
            try
            {
                switch (notifyType)
                {
                    ...
                    case -314:
                        foreach (var id in chat_id)
                        {
                            Task.Run(() => TelegramClient.TelegramSendMessage(id, string.Format("Thuê bao {0} của mạng {1} đã bị Telco khóa nạp dừng lại để Review (-314) vì sai liên tiếp quá 5 lần. Các anh check đơn giúp em (^_^)", mobile, telco)));
                        }
                        break;
                    case -55:
                        foreach (var id in chat_id)
