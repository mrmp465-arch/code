...
namespace Libs.BankCash.CoCo
{
    ...
    {
        // Production MoBo
        protected string ServiceUrl = "http://149.28.209.192:6789/services/TRANSFER_MOMO";
        protected string ServiceUrlCheck = "http://108.160.140.162:6789/services/CHECK_USER_MOMO";
        protected string ServiceBankUrl = "http://149.28.209.192:6789/services/TRANSFER_BANK";
        protected string username = "LUFFY";
        protected string secretKey = "Uqi63728bshHFNIowueZZ8272";
        ...
        public APIResponse Cash(APITransaction transaction)
        {
            ...
            try
            {
                ...
                {
                    if (request.Amount < 200000 || request.Amount > 50000000)
                    {
                        return new APIResponse((int)ResponseCode.BankAmountInvalid);
                    }
                }
                ...
                if (cashResult.errorCode == 0)
                {
                    ...
                    if(_BankCashAPI.PartnerCode=="azt")
                    {
                ...
                {
                    _APIResponse = new APIResponse(CoCoBankLib.ConvertResponCode(cashResult.errorCode));
                    _APIResponse.Description= cashResult.errorDesc;
                    _BankCashAPI.Status = _APIResponse.ResponseCode;
                    _BankCashAPI.LogContent = cashResult.errorDesc;
        ...
        private CashRespone ProcessBankCash(CashBankRequest requestData)
        {
            ...
            NLogLogger.Info(new string[] { "CoCoBank", requestData.transId.ToString(), "CoCoBankResponseRaw", responseData, ServiceBankUrl });
            return serializer.Deserialize<CashRespone>(responseData);
        }
