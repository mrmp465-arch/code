            ...
            if (string.IsNullOrEmpty(userName))
            {
                switch (providerCode)
                {
                    ...
                    case "shoappvtt":
                        ...
                        chat_id.Add(-416925073); //Chu
                        break;
                    case "vanappvtt":
                        chat_id.Add(-225466378);
                        break;
                    case "kenappvtt":
                        chat_id.Add(-421472488);
                        break;
                    default:
            ...
            try
            {
                switch (notifyType)
                {
                    ...
                    case -314:
                        foreach (var id in chat_id)
                        {
                            Task.Run(() => TelegramClient.TelegramSendMessage(id, string.Format("Thuê bao {0} của mạng {1} đã bị hệ thống khóa nạp dừng lại để Review (-314) vì sai liên tiếp quá 5 lần. Các anh check đơn giúp em (^_^)", mobile, telco)));
                        }
                        break;
                    case -2:
                        foreach (var id in chat_id)
