...
namespace APIMyViettel.GSM
{
    public class GSMServiceBiz
    {
        ...
        private static string callbackUrl = ConfigurationManager.AppSettings["CalbackUrl"] ?? "http://149.28.130.246:1585/GSMCallback.ashx";
        private static int cardValue = int.Parse(ConfigurationManager.AppSettings["GSMCard_Value_Required"] ?? "0");
        private static JavaScriptSerializer serializer = new JavaScriptSerializer();
        private static bool GetSimGSMStatus(string mobiNumber, string gsmServiceObj)
        {
        ...
        public static List<string> GetListSimGSMStatus(string gsmServiceObj)
        {
            var apiUrl = gsmServiceObj + "/api/GsmModem/GetAllDeviceInfo";
            var res = Task.Run(async () => await GetTask(apiUrl)).Result;
        ...
        public static APIResponse SendChardRequest(string transactionId, string telco, string sim, string simTarget, string cardSerial, string cardCode, int actionType, string gsmServiceObj, string accountName, string userName = "")
        {
            ...
            if (string.IsNullOrEmpty(simTarget))
            {
                ...
                {
                    Description = "Mobile or AccountName (Nếu khác Trả Sau cần khai báo mã gạch nợ tại trường AccountName)"
                };
            }
            ...
            NLogLogger.Info(new string[] { "GSMService", transactionId, "UssdResponse", res });
            if (!string.IsNullOrEmpty(res))
            {
