    ...
    protected void btUpdate_Click(object sender, EventArgs e)
    {
        ...
        else
        {
           ...
           // var partner = new Partners().GetCache(order.PartnerCode);
            ...
            _userLog.Add();
            TelegramClient.SendTeleV2("-1003968792146", "Cập nhật đúng lệnh nạp bank  " + _BankGateAPI.TotalAmount.ToString("#,#").Replace(",", ".") + " mã code " + _BankGateAPI.OrderNo + " RefCode " + _BankGateAPI.RefCode + " Từ tài khoản " + AppUtils.UserName);
            if (_BankGateAPI.Status == 1 && oldstatus < 1)
            {
                ...
                if (!string.IsNullOrEmpty(partner.SMSUrl))
                {
                    ...
                    if (rw > 0)
                    {
                        ...
                        var TotalR = _BankGateAPI.Reward;
                        ...
                        UpdatePartnerBalanceReward(partner.SMSUrl, TotalR, Balancedesc2, "RBankIn_" + _BankGateAPI.TransactionID.ToString());
                    }
                }
            }
            //if (_BankGateAPI.Status < 1 && oldstatus== 1)
