        ...
        private APIResponse Cash(APITransaction transaction)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(_partnerService.IPAddress))
                {
                    ...
                    {
                        TelegramNotify.SendTeleV2("-1003578695759", "Có lệnh rút bank sai ip từ đối tác " + transaction.PartnerCode + " :"+ IPAddress.Get());
                ...
                if (_partnerService.Quota != 0 && _partnerService.Occurs == 1)
                {
                    ...
                    //{
                    //    TelegramNotify.SendTeleV2("-1003578695759", "[Out Litmit] Vượt hạn mức rút từ đối tác " + transaction.PartnerCode + "(" + total.ToString("#,#").Replace(",", ".") + "-" + total2.ToString("#,#").Replace(",", ".")  +" Refcode "+request.RefCode + ")");
                    ...
                    //    {
                    //        TelegramNotify.SendTeleV2("-1003578695759", "[Warning Out Litmit] Đối tác " + transaction.PartnerCode + " sắp vượt hạn mức (" + total.ToString("#,#").Replace(",", ".") + "-" + total2.ToString("#,#").Replace(",", ".") + ")");
                    //    }
                    //}    
