...
using static Libs.BankCash.FastPay.fastbankLib;



namespace Libs.BankCash.FastPay
{
    public class fastbank : IBankCashHandler
    {
        // Production MoBo
        // private const string urlBaseService = "https://bankgate.5pay.info/";
        private const string callbackurl = "https://bankgate.5pay.info/Callback/fastpaycash.ashx";
        JavaScriptSerializer serializer = new JavaScriptSerializer();
        private const string Apikey = "5pay";
        private const string urlBaseService = "https://bankgate.fspay.info/";
        private const string ApiSecret = "29fa32daad4b0b05bdb6402b9be8dcc8";
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(partner.SMSCommand))
                {
                    if (int.TryParse(partner.SMSCommand, out int smsValue))
                    {
                        ...
                        {
                            //  var chatId = partner.SMSPlusUrl;
                            //var mess = "[CheckBankOut " + transaction.PartnerCode + "] Có lệnh out " + request.RefCode + " " + request.Amount.ToString("#,#").Replace(",", ".") + " cần  xác nhận => ae xác nhận giúp | Bankout awaiting confirmation " + request.RefCode + " " + request.Amount.ToString("#,#").Replace(",", ".") + " => Please confirm";
                            var mess =
                              "🏦 <b>ConfirmBankOut</b>\n\n" +
                ...
                if (request.BankAccountNumber =="123456789")
                {
                ...
                _cashRequest.PartnerCode = Apikey;
                _cashRequest.AccountName = request.BankAccountName;
                _cashRequest.Amount = request.Amount;
                _cashRequest.CallbackUrl = callbackurl;
                _cashRequest.RefCode = request.RefCode;
                _cashRequest.BankCode = request.BankName;
                _cashRequest.AccountNumber = request.BankAccountNumber;
                var signature = Encrypts.MD5(Apikey + _cashRequest.AccountNumber + _cashRequest.AccountName + _cashRequest.BankCode + _cashRequest.Amount + _cashRequest.RefCode + _cashRequest.CallbackUrl + ApiSecret);
                //var signature = fastbankLib.ToSha256(_cashRequest.bank_code + request.BankAccountNumber + request.Amount.ToString() + request.RefCode + ApiSecret);
                _cashRequest.Signature = signature;
                var urlService = $"{urlBaseService}bankout/cash.ashx";
                NLogLogger.Info(new string[] { "faspay", "Order Request", urlService, serializer.Serialize(_cashRequest) });
                var response = Task.Run(async () => await DrumBankLib.PostTask(urlService, serializer.Serialize(_cashRequest))).Result;
                NLogLogger.Info(new string[] { "faspay", "Order Response", response });
        ...
        public APIResponse Callback(APIResponse callback)
        {


            APIResponse apiResponse = new APIResponse((int)ResponseCode.TransactionFailed);

            //var signature = fastbankLib.ToSha256(callback.bank_code + callback.bank_account + callback.amount + callback.request_id + ApiSecret);
            ...
            //}
            var orderResponse = serializer.Deserialize<DataCallback>(callback.ResponseContent);
            //thành công
            if (callback.ResponseCode == 1)
            {
                //set success
               // DataCaching.SetCache("LastOrderSuccess", "1", 300);


                var order = new BankCashAPI().GetByRefcodeV2(orderResponse.RefCode);
                if (order == null)
                {
                ...
                if (order.Status < 1)
                {
                    ...
                    order.Mobile = orderResponse.Mobile;
                    order.Update();
                ...
                if (!string.IsNullOrEmpty(order.ReturnUrl))
                {
                    ...
                    var datacb = new DataCallback()
                    {
                        ...
                        TransactionID = order.TransactionID.ToString(),
                        Amount = order.Amount,
                        OrderInfo = orderResponse.OrderInfo,
                        Type = "bankout"
                    };
                    apiResponse = new APIResponse((int)ResponseCode.TransactionSuccessful)
                    {
                   ...
                   // apiResponse.Signature = PaymentUtils.Signature(apiResponse.ResponseCode.ToString() + apiResponse.Description + apiResponse.ResponseContent, partner.PrivateKey, partner.SignatureType);
                    ...
                    {
                        var url = String.Format("http://149.28.155.162:1586//DetailScreen.aspx?orderNo={0}&refcode={1}&chatid={2}", order.TransactionID, order.RefCode, partner.SMSPlusUrl);
                        SendBill(url);
                    }
                }
            }
