    ...
    public class VPGJsonService : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                var request = javaScriptSerializer.Deserialize<RequestData>(jsonString);
                NLogLogger.Info(new string[] { "VPGJsonService", "Request", jsonString });
                result = VPGUtils.Request(request.PartnerCode, request.ServiceCode, request.CommandCode, request.RequestContent, request.Signature);
            }
            catch (Exception exp)
            {
