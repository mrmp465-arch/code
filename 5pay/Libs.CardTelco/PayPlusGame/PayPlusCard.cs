    ...
    public class PayPlusCard : ICardTelcoHandler
    {
        ...
        public APIResponse UseCard(APITransaction transaction)
        {
            ...
            try
            {
                ...
                //Nếu không phải thẻ Gate thì check đầu vào AmountUser
                ...
                while ((cardResult.code == (int)ResponseCode.ServiceIsLocked || cardResult.code == (int)ResponseCode.TransactionLimit) && tryAgain < 1)
                {
                ...
                if (cardResult.code == 1)
                {
                    ...
                    else // Trường hợp AmountUser = 0 tức là đã Pass đầu vào nên ko cần phải check sai mệnh giá
                    {
                        _APIResponse = new APIResponse((int)ResponseCode.TransactionSuccessful);
                    }
