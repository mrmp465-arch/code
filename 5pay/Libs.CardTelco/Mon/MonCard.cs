...
namespace Libs.CardTelco.Mon
{
    public class MonCard : ICardTelcoHandler
    {

        protected string providerName = string.Empty;
        string serviceUrl = "http://127.0.0.1:1594/";
        ...
        public APIResponse UseCard(APITransaction transaction)
        {
            ...
            try
            {
                ...
                if (_CardAPILog.ReturnValue < 0)
                {
                    ...
                    {
                        NLogLogger.Info(new string[] { "MonCard", transaction.TransactionID.ToString(), transaction.PartnerCode, "TopupRequest", request.CardSerial, request.CardCode, "Error Insert Data" });
                ...
                NLogLogger.Info(new string[] { "MonCard", transaction.TransactionID.ToString(), "TopupRequest", serializer.Serialize(topupRequest) });
                TopupResponse cardResult = ProcessCard(topupRequest);
                NLogLogger.Info(new string[] { "MonCard", transaction.TransactionID.ToString(), "TopupResponse", serializer.Serialize(cardResult) });
                ...
                else
                {
                    switch (cardResult.code)
                    {
                        ...
                        case (int)ResponseCode.TransactionTimeout:
                            ...
                            _CardAPILog.Status = _APIResponse.ResponseCode;
                            _APIResponse.ResponseContent = "mon";
                            break;
            ...
            catch (Exception ex)
            {
                ...
                {
                    case 1:
                        NLogLogger.Info(new string[] { "MonCard", transaction.TransactionID.ToString(), "Error", "Step1", ex.Message.Replace("\n", " ") });
                        _APIResponse = new APIResponse((int)ResponseCode.RequestContentInvalid);
                        break;
                    case 2:
                        NLogLogger.Info(new string[] { "MonCard", transaction.TransactionID.ToString(), "Error", "Step2", ex.Message.Replace("\n", " ") });
                        _APIResponse = new APIResponse((int)ResponseCode.SystemError);
                        break;
                    case 3:
                        NLogLogger.Info(new string[] { "MonCard", transaction.TransactionID.ToString(), "Error", "UseCard", ex.Message.Replace("\n", " ") });
                        ...
                        break;
                    case 4:
                        NLogLogger.Info(new string[] { "MonCard", transaction.TransactionID.ToString(), "Error", "Step4", ex.Message.Replace("\n", " ") });
        ...
        {
            JavaScriptSerializer serializer = new JavaScriptSerializer();
            //NLogLogger.Info(new string[] { "MonCard", "TopupRequest", serializer.Serialize(topupRequest) });
            ...
            //NLogLogger.Info(new string[] { "MonCard", "TopupResponse", serviceResponse });
            ...
            {
                NLogLogger.Info(new string[] { "MonCard", "MonCardTopupRequest", "TimeoutException", topupRequest.transactionId.ToString(), e.Message });
                Thread.ResetAbort();
            ...
            {
                NLogLogger.Info(new string[] { "MonCard", "MonCardTopupRequest", "ThreadAbortException", topupRequest.transactionId.ToString(), e.Message });
                Thread.ResetAbort();
                return new TopupResponse()
                {
                    code = (int)ResponseCode.TransactionTimeout,
            ...
            {
                NLogLogger.Info(new string[] { "MonCard", "MonCardTopupRequest", "Exception", topupRequest.transactionId.ToString(), e.Message });
                return new TopupResponse()
                {
                    code = (int)ResponseCode.TransactionTimeout,
            ...
            try
            {
                NLogLogger.Info(new string[] { "MonCard", "MonCardTopupRequest", topupRequest.transactionId.ToString(), serviceResponse });
                return serializer.Deserialize<TopupResponse>(serviceResponse);
            }
            catch (Exception e)
            {
                NLogLogger.Info(new string[] { "MonCard", "MonCardTopupRequest", topupRequest.transactionId.ToString(), e.Message });
                return new TopupResponse()
                {
                    code = (int)ResponseCode.TransactionTimeout,
