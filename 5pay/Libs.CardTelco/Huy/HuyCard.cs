...
namespace Libs.CardTelco.Huy
{
    public class HuyCard : ICardTelcoHandler
    {

        protected string providerName = string.Empty;
        string serviceUrl = "http://127.0.0.1:1586/";
        ...
        public APIResponse UseCard(APITransaction transaction)
        {
            ...
            try
            {
                ...
                if (_CardAPILog.ReturnValue < 0)
                {
                    ...
                    {
                        NLogLogger.Info(new string[] { "HuyCard", transaction.TransactionID.ToString(), transaction.PartnerCode, "TopupRequest", request.CardSerial, request.CardCode, "Error Insert Data" });
                ...
                NLogLogger.Info(new string[] { "HuyCard", transaction.TransactionID.ToString(), "TopupRequest", serializer.Serialize(topupRequest) });
                TopupResponse cardResult = ProcessCard(topupRequest);
                NLogLogger.Info(new string[] { "HuyCard", transaction.TransactionID.ToString(), "TopupResponse", serializer.Serialize(cardResult) });
                ...
                else
                {
                    switch (cardResult.code)
                    {
                        ...
                        case (int)ResponseCode.TransactionTimeout:
                            ...
                            _CardAPILog.Status = _APIResponse.ResponseCode;
                            _APIResponse.ResponseContent = "huy";
                            break;
            ...
            catch (Exception ex)
            {
                ...
                {
                    case 1:
                        NLogLogger.Info(new string[] { "HuyCard", transaction.TransactionID.ToString(), "Error", "Step1", ex.Message.Replace("\n", " ") });
                        _APIResponse = new APIResponse((int)ResponseCode.RequestContentInvalid);
                        break;
                    case 2:
                        NLogLogger.Info(new string[] { "HuyCard", transaction.TransactionID.ToString(), "Error", "Step2", ex.Message.Replace("\n", " ") });
                        _APIResponse = new APIResponse((int)ResponseCode.SystemError);
                        break;
                    case 3:
                        NLogLogger.Info(new string[] { "HuyCard", transaction.TransactionID.ToString(), "Error", "UseCard", ex.Message.Replace("\n", " ") });
                        _CardAPILog.Description = "Error - UseCard " + ex.Message.Replace("\n", " ");
                        _APIResponse = new APIResponse((int)ResponseCode.TransactionSuspicious);
                        break;
                    case 4:
                        NLogLogger.Info(new string[] { "HuyCard", transaction.TransactionID.ToString(), "Error", "Step4", ex.Message.Replace("\n", " ") });
        ...
        {
            JavaScriptSerializer serializer = new JavaScriptSerializer();
            //NLogLogger.Info(new string[] { "HuyCard", "TopupRequest", serializer.Serialize(topupRequest) });
            ...
            //NLogLogger.Info(new string[] { "HuyCard", "TopupResponse", serviceResponse });
            ...
            {
                NLogLogger.Info(new string[] { "APIHuy", "Request", "TimeoutException", topupRequest.transactionId.ToString(), e.Message });
                Thread.ResetAbort();
            ...
            {
                NLogLogger.Info(new string[] { "APIHuy", "Request", "ThreadAbortException", topupRequest.transactionId.ToString(), e.Message });
                Thread.ResetAbort();
                return new TopupResponse()
                {
                    code = (int)ResponseCode.TransactionTimeout,
            ...
            {
                NLogLogger.Info(new string[] { "APIHuy", "Request", "Exception", topupRequest.transactionId.ToString(), e.Message });
                return new TopupResponse()
                {
                    code = (int)ResponseCode.TransactionTimeout,
            ...
            try
            {
                NLogLogger.Info(new string[] { "APIHuy", "Request", topupRequest.transactionId.ToString(), serviceResponse });
                return serializer.Deserialize<TopupResponse>(serviceResponse);
            }
            catch (Exception e)
            {
                NLogLogger.Info(new string[] { "APIHuy", "Request", topupRequest.transactionId.ToString(), e.Message });
                return new TopupResponse()
                {
                    code = (int)ResponseCode.TransactionTimeout,
