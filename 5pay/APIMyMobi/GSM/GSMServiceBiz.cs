...
namespace APIMyMobi.GSM
{
    public class GSMServiceBiz
    {
        ...
        private static string callbackUrl = ConfigurationManager.AppSettings["CalbackUrl"] ?? "http://149.28.130.246:1591/GSMCallback.ashx";
        private static JavaScriptSerializer serializer = new JavaScriptSerializer();
        private static bool GetSimGSMStatus(string mobiNumber, string gsmUrl)
        {
            var apiUrl = gsmUrl + "GetAllDevicesInfo";
            NLogLogger.Info(new string[] { "GSMService", "Request GetSimGSMStatus", apiUrl, mobiNumber });
            var res = Task.Run(async () => await GetTask(apiUrl)).Result;
            NLogLogger.Info(new string[] { "GSMService", "Response GetSimGSMStatus", apiUrl, mobiNumber, res });
            ...
            if (!string.IsNullOrEmpty(res))
            {
                ...
                {
                    if (mobiNumber == r.PhoneNumber && r.State == 1)
                    {
                        return true;
                    }
                }
        ...
        public static List<string> GetListSimGSMStatus(string gsmUrl)
        {
            ...
            if (!string.IsNullOrEmpty(res))
            {
                ...
                foreach (var r in response.ResponseContent)
                {
                    if (r.State == 1 && !string.IsNullOrEmpty(r.PhoneNumber))
                    {
                        lstSim.Add(r.PhoneNumber);
                    }
                }
        ...
        public static APIResponse SendChardRequest(string transactionId, string telco, string sim, string simTarget, string cardSerial, string cardCode, int actionType, string gsmUrl)
        {
            ...
            if (GetSimGSMStatus(sim, gsmUrl))
            {
                ...
                NLogLogger.Info(new string[] { "GSMService", "UssdRequest", apiUrl, serializer.Serialize(request) });
                var res = Task.Run(async () => await PosTask(apiUrl, JsonConvert.SerializeObject(request))).Result;
                NLogLogger.Info(new string[] { "GSMService", "UssdResponse", apiUrl, serializer.Serialize(request), res });
                if (!string.IsNullOrEmpty(res))
                {
