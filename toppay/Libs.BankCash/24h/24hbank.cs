...
namespace Libs.BankCash._24h
{
    public class _24hBank : IBankCashHandler
    {
        ...
        private const string Apikey = "fast";

        private const string ApiSecret = "b8aef1658f40d3716573f588d5f429d0";
        ...
        public APIResponse Cash(APITransaction transaction)
        {
            ...
            try
            {
                ...
                if (request.Amount >= MinBankAproveAmount || transaction.PartnerCode == "pp1")
                {
                    TelegramNotify.SendTeleV2("-4762440012", "[Duyệt] Có lệnh bankout cần duyệt  " + request.Amount.ToString("#,#").Replace(",", ".") + " từ đối tác " + transaction.PartnerCode + " RefCode " + request.RefCode);
                ...
                _cashRequest.RefCode = _BankCashAPI.ReturnValue.ToString();
                _cashRequest.BankCode = request.BankName;
                _cashRequest.AccountNumber = request.BankAccountNumber;
                
                var signature = Utils.Encrypts.MD5(Apikey +request.BankAccountNumber+ request.BankAccountName+request.BankName+ request.Amount.ToString() + _BankCashAPI.ReturnValue.ToString()+callbackurl + ApiSecret);
                ...
                else
                {
                    ...
                    _BankCashAPI.Status = -1;
                    _BankCashAPI.LogContent = serializer.Serialize(_APIResponse.Description);
                    _BankCashAPI.LastTime = DateTime.Now;
                    _BankCashAPI.Update();
        ...
        public APIResponse Callback(_24hBankLib.Callback callback)
        {
            ...
            var signature = Utils.Encrypts.MD5(callback.ResponseCode + callback.Description + callback.RefCode + ApiSecret);
            if (signature != callback.Signature)
            {
                NLogLogger.Info(new string[] { "24h", "Callback", "Signature Failed", signature, callback.Signature });
