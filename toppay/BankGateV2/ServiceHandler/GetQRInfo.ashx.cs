    ...
    public class GetQRInfo : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (type == 1)
            {
                ...
                if (string.IsNullOrEmpty(bankinfo.bankAccountNumber))
                    bankinfo.responsecode = -1;
                else
                    bankinfo.bankAccountName = GetBankAccount(bankinfo.bankCode, bankinfo.bankAccountNumber);


                NLogLogger.Info(new string[] { "QRCode", "Respone", serializer.Serialize(bankinfo) });

                if (bankinfo.responsecode == -1)
                    NLogLogger.Info(new string[] { "QRCode", "Request", request.qrtext });
            ...
            if (type == 2)
            {
                ...
                if (bankinfo.responsecode == -1)
                    bankinfo = getBankinfo(request.qrtext);
            ...
            if (type == 3)
            {
                ...
                if (string.IsNullOrEmpty(bankinfo.bankAccountNumber))
                {
                    bankinfo = getBankinfo2(request.qrtext);
                }
                if (string.IsNullOrEmpty(bankinfo.bankAccountNumber))
                {
                    bankinfo = getBankinfo(request.qrtext);
            ...
            if (type == 4)
            {
                ...
                if (string.IsNullOrEmpty(bankinfo.bankAccountNumber))
                {
                    bankinfo = getBankinfo4b(request.qrtext);
                }
            ...
            try
            {
                ...
                if (tags.TryGetValue("38", out tag38))
                {
                    ...
                    if (string.IsNullOrEmpty(proxyId))
                    {
                        foreach (var sub in subFields.Values)
                        {
                            var match = Regex.Match(sub.Value, @"(99)?MM\d+[A-Z]\d+");
                            if (match.Success)
                            {
                                proxyId = match.Value;
                                break;
        ...
        public BankInfo getBankinfo4b(string qrData)
        {
            var result = new BankInfo { responsecode = -1 };
            ...
            try
            {
                ...
                // 2) Tag 38: VietQR domestic account info (chứa BIN + proxy)
                string proxyId = "";
                string bankBin = "";
                ...
                if (tags.TryGetValue("38", out tag38))
                {
                    Dictionary<string, QRField> t38 = ParseTLV(tag38.Value);

                    QRField t38_01;
                    if (t38.TryGetValue("01", out t38_01))
                    {
                        Dictionary<string, QRField> t38_01_sub = ParseTLV(t38_01.Value);

                        QRField binField;
                        if (t38_01_sub.TryGetValue("00", out binField))
                            bankBin = binField.Value;

                        QRField proxyField;
                        if (t38_01_sub.TryGetValue("01", out proxyField))
                            proxyId = proxyField.Value;
                    }
                }

                // 3) Amount: chỉ đọc được nếu có tag54 top-level (QR bạn gửi hiện KHÔNG có)
                QRField t54;
                string amountRaw = "";
                if (tags.TryGetValue("54", out t54))
                {
                    amountRaw = t54.Value;
                }
                int amount = 0;
                if (!string.IsNullOrEmpty(amountRaw))
                    int.TryParse(amountRaw, out amount);

                // 4) Map bank bin -> bank code/name
                var lstbankcode = new BankCodeTranfer().GetListCache();
                var bank = lstbankcode.FirstOrDefault(x => x.bin == bankBin);

                result.bankAccountNumber = proxyId;
                result.countryCode = "VN";
                result.bankCode = bank?.code ?? "";
                result.bankName = bank?.shortName ?? "";
        ...
        private Dictionary<string, QRField> ParseTLV(string data)
        {
            var dict = new Dictionary<string, QRField>();
            int i = 0;

            while (i + 4 <= data.Length)
            {
                string id = data.Substring(i, 2);
                int len = int.Parse(data.Substring(i + 2, 2));

                int start = i + 4;
                if (start + len > data.Length) break;

                string val = data.Substring(start, len);
                dict[id] = new QRField { ID = id, Length = len, Value = val };
