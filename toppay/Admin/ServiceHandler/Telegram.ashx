            ...
            try
            {
                ...
                if (resObj.message.text.StartsWith("/bill"))
                {
                    ...
                    if (checkorder != null)
                    {
                        ...
                        //    ResponseContent = javaScriptSerializer.Serialize(checkorder)
                        //};
                        var url = String.Format("http://149.28.155.162:1586/DetailScreen5.aspx?orderNo={0}&refcode={1}&chatid={2}", checkorder.TransactionID, checkorder.RefCode, resObj.message.chat.id.ToString());
                        SendBill(url);
                        //SendTeleV2(resObj.message.chat.id.ToString(), "Transaction success");
                    ...
                    if (trans.Status >= 1)
                    {
                        var url = String.Format("http://149.28.155.162:1586/DetailScreen5.aspx?orderNo={0}&refcode={1}&chatid={2}", trans.TransactionID, trans.RefCode, resObj.message.chat.id.ToString());
                        SendBill(url);
                        //SendTeleV2(resObj.message.chat.id.ToString(), "Transaction success");
                        return;
                    }
