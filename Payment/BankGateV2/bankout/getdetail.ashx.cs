    ...
    public class getdetail : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                // Kiểm tra chữ ký
                if (!PaymentUtils.CheckSignature(_Partner.PartnerCode + request.RefCode, request.Signature, _Partner.PublicKey, _Partner.SignatureType))
                {

                    context.Response.Write(ResponseUtils.Response((int)ResponseCode.SignatureInvalid));
                    return;
                }
                var checkorder = DataCaching.GetCache<CheckOrder>("CheckOutOrder:" + request.PartnerCode + request.RefCode);
                if (checkorder != null)
                {
                ...
                var trans = new BankCashAPI().GetByRefcode(request.RefCode, request.PartnerCode);
                ...
                else
                {
                    if (trans.Status == -1)
                    {
                        ...
                        {
                            context.Response.Write(ResponseUtils.Response((int)ResponseCode.BankAccountInvalid));
                            return;
                        }
                        context.Response.Write(ResponseUtils.Response((int)ResponseCode.TransactionFailed));
                        return;
                    }
                    if (trans.Status == -357)
                    {
