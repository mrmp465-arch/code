...
namespace BankGateV2.ServiceHandler
{
    ...
    /// </summary>
    public class GetBankImage : IHttpHandler
    {

        public void ProcessRequest(HttpContext context)
        {
            JavaScriptSerializer serializer = new JavaScriptSerializer();
            var bankId = HttpContext.Current.Request.QueryString["bankId"];
            var bankCode = HttpContext.Current.Request.QueryString["bankCode"];
            try
            {
                //NLogLogger.Info(new string[] { "GetImg ", bankId.ToString() });
                if (!string.IsNullOrEmpty(bankId))
                {
                    var bank = new BankAccounts().Get(bankId, bankCode);
                    if (bank != null)
                    {
