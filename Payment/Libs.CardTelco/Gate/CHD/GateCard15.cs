...
{
    public class GateCard15 : ICardTelcoHandler
    {

        // Cấu hình kết nối Production 
        protected string webserviceUrl = "https://ops.gate.vn:8888/Igate_WS/Route?wsdl";

        protected string merchantID = "1331";
        protected string secretKey = "14cd11b1a9130dc122e3b26cc8cf0079";
        protected string PrivateKey = "c804fbe6";
        protected string KeyPath = "\\1331.p12";
        protected string telcoservicecode = "10013319999";

        protected JavaScriptSerializer serializer = new JavaScriptSerializer();
        protected string providerName = "gate15";
        ...
        public APIResponse UseCard(APITransaction transaction)
        {
            ...
            try
            {
                ...
                rq.TelcoServiceCode = telcoservicecode;

                String originalData = String.Format("{0}{1}{2}{3}{4}{5}", rq.MerchantID, rq.Username, rq.CardSerial, rq.CardPIN, rq.TelcoServiceCode, secretKey);
                NLogLogger.Info(new string[]
                {cardType, transaction.TransactionID.ToString(), "GateRequest", merchantID.ToString(), originalData});
                ...
                string gateResponse = sb.ProcessRequest(cardType, xmlRequestData);
                //Output : Trả về giá trị của chuỗi với định dạng (ErrorCode|Description|TransactionID|PartnerTransactionID|CardAmount) 
                NLogLogger.Info(new string[]
                {"Gate", transaction.TransactionID.ToString(), "GateResponse", gateResponse});
            ...
            catch (Exception ex)
            {
                switch (step)
                {
                    case 1:
                        NLogLogger.Info(new string[]
                        {
                            "Gate", transaction.TransactionID.ToString(), "Error", "UseCard", "Step1",
                            ex.Message.Replace("\n", " ")
                        });
                    ...
                    case 2:
                        NLogLogger.Info(new string[]
                        {
                            "Gate", transaction.TransactionID.ToString(), "Error", "UseCard", "Step2",
                            ex.Message.Replace("\n", " ")
                        });
                    ...
                    case 3:
                        NLogLogger.Info(new string[]
                        {
                            "Gate", transaction.TransactionID.ToString(), "Error", "UseCard", "Step3",
                            ex.Message.Replace("\n", " ")
                        });
                    ...
                    case 4:
                        NLogLogger.Info(new string[]
                        {
                            "Gate", transaction.TransactionID.ToString(), "Error", "UseCard", "Step4",
                            ex.Message.Replace("\n", " ")
                        });
                    ...
                    case 5:
                        NLogLogger.Info(new string[]
                        {
                            "Gate", transaction.TransactionID.ToString(), "Error", "UseCard", "Step5",
                            ex.Message.Replace("\n", " ")
                        });
                    ...
                    case 6:
                        NLogLogger.Info(new string[]
                        {
                            "Gate", transaction.TransactionID.ToString(), "Error", "UseCard", "Step6",
                            ex.Message.Replace("\n", " ")
                        });
                    ...
                    default:
                        NLogLogger.Info(new string[]
                        {
                            "Gate", transaction.TransactionID.ToString(), "Error", "UseCard", ex.Message.Replace("\n", " ")
                        });
