...
namespace Libs.BankDirect.Simex
{
    public class SimexBank : IBankDirectV2Handler
    {


        private const string urlBaseService = "https://api.bobap.xyz/api/";
        private const string callbackurl = "https://bankgate.coroach.xyz//Callback/Simex.ashx";
        JavaScriptSerializer serializer = new JavaScriptSerializer();
        private const string Apikey = "2c4bc033e05e45db807ab9b2ff07d165";
        ...
        private List<BankAccountV2> GetBanksAPI()
        {
            ...
            var result = DataCaching.GetCache<List<BankAccountV2>>("SimexBank");
            if (result != null)
            {
                return result;
            }
            ...
            if (!string.IsNullOrEmpty(response))
            {
                ...
                if (resObj != null)
                {
                    ...
                    DataCaching.SetCache("SimexBank", listBankObj, 60 * 1);
                    return listBankObj;
                }
        ...
        public APIResponse Order(APITransaction transaction)
        {
            ...
            if (add > 0)
            {
                ...
                var signature = Utils.Encrypts.MD5(Apikey + add + tran.Amount);
                var urlService = $"{urlBaseService}V2/RegCharge?apiKey={Apikey}&chargeType=IN&amount={tran.Amount}&requestId={add}&member_identity={add}&bankCode={tran.BankName}&callback_url={callbackurl}&signature={signature}";
                ...
                if (!string.IsNullOrEmpty(response))
                {
                    ...
                    if (resObj.stt == 1)
                    {
                        ...
                        addTran.BankAccountName = resObj.data.accountName;
                        addTran.BankAccountNumber = resObj.data.accountNo;
                        addTran.OrderNo = resObj.data.transferContent;
                        addTran.UpdateBank();
                        ...
                        bankinfo.OrderNo = resObj.data.transferContent;
                        bankinfo.Status = "0";
                        bankinfo.BankId = resObj.data.accountNo;
                        bankinfo.BankName = resObj.data.accountName;
                        ...
                        var orderRes = new Order()
                        {
                            ...
                            Amount = tran.Amount,
                            RefCode = tran.RefCode,
                            OrderNo = resObj.data.transferContent,
                            Timeout = 15,
                            QRCode = String.Format("https://img.vietqr.io/image/{0}-{1}-compact.jpg?amount={2}&addInfo={3}", addTran.BankCode, resObj.data.accountNo, resObj.data.amount, resObj.data.transferContent),
                            Url = String.Format("https://img.vietqr.io/image/{0}-{1}-compact.jpg?amount={2}&addInfo={3}", addTran.BankCode, resObj.data.accountNo, resObj.data.amount, resObj.data.transferContent),
                            BankName = tran.BankName,
                            BankAccountNumber = resObj.data.accountNo,
                            BankAccountName = resObj.data.accountName
                        };
        ...
        public APIResponse Callback(SimexBankLib.Callback callback)
        {
            ...
            var signature = Utils.Encrypts.MD5(Apikey + callback.requestId + callback.amount +  callback.status + callback.bankCode);
            ...
            if (order.Status != (int)ResponseCode.TransactionSuccessful)
            {
                ...
                order.OrderInfo = callback.requestId;
                ...
                if (resultupdate >= 0)
                {
                    ...
                    if (!string.IsNullOrEmpty(order.ReturnUrl))
                    {
                        ...
                        //    datacb2.Amount = Convert.ToInt32(callback.amount);
                        //    datacb2.Type = "bank";
