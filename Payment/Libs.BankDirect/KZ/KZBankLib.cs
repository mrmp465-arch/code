...
namespace Libs.BankDirect.KZ
{
    public class KZBankLib
    {
        ...
        public static async Task<string> CallbackJson(string url, string postData, long Id = 0,string refcode="")
        {
            ...
            NLogLogger.Info(new string[] { "kz", "Callback", "Partner", "Request", postData, url });
            var uri = new Uri(url);
            var httpContent = new StringContent(postData, Encoding.UTF8, "application/json");
            httpContent.Headers.ContentType = new MediaTypeHeaderValue("application/json");
            ...
            try
            {
                ...
                if (response.Content != null)
                {
                    ...
                    NLogLogger.Info(new string[] { "kz", "Callback", "TransId","Refcode", "Response", Id.ToString(),refcode, responseContent });
            ...
            catch (WebException e)
            {
                ...
                if (responseStream != null)
                {
                    ...
                    {
                        NLogLogger.Info(new string[] { "kz", "Exeption Post", reader.ReadToEnd() });
                ...
                NLogLogger.Info(new string[] { "kz", "Exeption Post", e.Message });
                var log = new LogInfo
                {
                    LogTime = DateTime.Now,
                    Url = url,
