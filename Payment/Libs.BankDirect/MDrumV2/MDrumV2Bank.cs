...
namespace Libs.BankDirect.MDrumV2
{
    public class MDrumV2Bank : IBankDirectV2Handler
    {
        ...
        public List<BankAccountV4> GetBanksAPIV4(string ParnerCode)
        {
            ...
            if (ParnerCode == "vs8")
            {
                data = data.Where(x => x.PartnerName == "VS8").ToList();
            }
            else
            {
                if (ParnerCode == "qx88")
                {
                    data = data.Where(x => x.PartnerName == "QX88").ToList();
                }
                else
                {
                    if (ParnerCode == "tn99")
                    {
                        data = data.Where(x => x.PartnerName == "TN99").ToList();
                    }
                    else
                    {
                        data = data.Where(x => x.PartnerName == "ALL").ToList();
                    }
            ...
            //var listBankObj = new List<BankAccountV4>();
            var BankCodeInMaintain = systemDataConfig.GetKey(lstConfig, "BankCodeInMaintain");
            foreach (var item in data.Where(x => x.Id != 155))
            {
                var bankitem = new BankAccountV4()
                {
            ...
            //if (DateTime.Now.Hour == 20 && DateTime.Now.Minute <= 6)
            //{
            //    listBank = listBank.Where(x => x.BankName != "BIDV").ToList();
            //}
            //if (DateTime.Now.Hour < 2 || DateTime.Now.Hour >= 22)
            //{
            //    listBank = listBank.Where(x => x.BankName != "BIDV").ToList();
            //}
            // 16:45 → 18:30
            var now = DateTime.Now.TimeOfDay;
            bool isBlockTime = (now >= new TimeSpan(16, 29, 0) && now <= new TimeSpan(18, 30, 0))
                ||
            // 22:30 → 01:30 (qua ngày)
            (now >= new TimeSpan(22, 30, 0) || now <= new TimeSpan(1, 30, 0));
        ...
        public List<BankAccountV2> GetBanksAPI(string partnercode)
        {
            ...
            if (!string.IsNullOrEmpty(response))
            {
                ...
                if (resObj.ResponseCode > 0)
                {
                    ...
                    bool isBlockTime =
                     // 16:30 → 18:30
                     (now >= new TimeSpan(16, 30, 0) && now <= new TimeSpan(18, 30, 0))
                     ||
                     // 22:30 → 01:30 (qua ngày)
                     (now >= new TimeSpan(22, 30, 0) || now <= new TimeSpan(1, 30, 0));
        ...
        public APIResponse GetBanks(string partnercode)
        {
            ...
            if (partnercode == "vs8")
            {
                requestData.PartnerCode = "vs8";
            }
            if (partnercode == "qx88")
            {
                requestData.PartnerCode = "qx88";
            }
            if (partnercode == "tn99")
            {
                requestData.PartnerCode = "tn99";
            }
            ...
            if (!string.IsNullOrEmpty(response))
            {
                ...
                if (resObj.ResponseCode > 0)
                {
                    ...
                    if (listBankObj.Exists(x => x.BankName == "ACB"))
                    {
                        listBankObj = listBankObj.Where(x => x.BankName != "ACB").ToList();
                        listBankObj.Insert(0, new BankAccountV2()
                        {
                    ...
                    //if (partnercode != "akb")
                    //{
                    //    listBankObj = listBankObj.Where(x => x.BankName != "TIMO").ToList();
                    //    listBankObj = listBankObj.Where(x => x.BankName != "VCB").ToList();
                    //    listBankObj = listBankObj.Where(x => x.BankName != "ICB").ToList();
                    //    listBankObj = listBankObj.Where(x => x.BankName != "MB").ToList();
                    //    listBankObj = listBankObj.Where(x => x.BankName != "SEAB").ToList();
                    //    listBankObj = listBankObj.Where(x => x.BankName != "OCB").ToList();
                    //    listBankObj = listBankObj.Where(x => x.BankName != "BIDV").ToList();
                    //}
                    ...
                    if (resObj.ResponseCode > 0)
                    {
                        ...
                        if (add > 0)
                        {
                            ...
                            var orderRes = new Order()
                            {
                                ...
                                //LinkWebView = String.Format("https://momo.opaps.info/Pages/Momo.aspx?orderNo={0}", add.ToString()),
                                //Url = String.Format("https://momo.opaps.info/Pages/Momo.aspx?orderNo={0}", add.ToString()),
                                BankName = tran.BankName,
                                BankAccountNumber = bankinfo.MomoId,
                                BankAccountName = bankinfo.MomoName
                            };
                            if (tran.BankName.ToUpper() != "MOMO")
                            {
        ...
        public async Task<string> LogInfo(BankAccountReceive bankinfo)
        ...
        public APIResponse Order(APITransaction transaction)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (DateTime.Now.Hour < 2 || DateTime.Now.Hour >= 22)
                    {
                        if (tran.BankName.ToUpperInvariant() == "BIDV")
                            return new APIResponse((int)ResponseCode.BankCodeMaintain);
                    }
                    var BankCodeInMaintain = systemDataConfig.GetKey(lstConfig, "BankCodeInMaintain");
                    var BanksAPI = GetBanksAPIV4(transaction.PartnerCode);
                    if (transaction.PartnerCode == "vs8")
                    {
                        var BankCodeVS8 = systemDataConfig.GetKey(lstConfig, "BankCodeVS8").ToUpper();
                        BanksAPI = BanksAPI.Where(x => x.BankName != "TIMO").ToList();
                        if (!BankCodeVS8.Contains("VCB"))
                            BanksAPI = BanksAPI.Where(x => x.BankName != "VCB").ToList();

                        if (!BankCodeVS8.Contains("ICB"))
                            BanksAPI = BanksAPI.Where(x => x.BankName != "ICB").ToList();
                        if (!BankCodeVS8.Contains("MB"))
                            BanksAPI = BanksAPI.Where(x => x.BankName != "MB").ToList();
                        if (!BankCodeVS8.Contains("SEAB"))
                            BanksAPI = BanksAPI.Where(x => x.BankName != "SEAB").ToList();
                        if (!BankCodeVS8.Contains("OCB"))
                            BanksAPI = BanksAPI.Where(x => x.BankName != "OCB").ToList();
                        if (!BankCodeVS8.Contains("BIDV"))
                            BanksAPI = BanksAPI.Where(x => x.BankName != "BIDV").ToList();
                    }
                    //random
                ...
                if (transaction.PartnerCode == "qx88")
                {
                    requestData.PartnerCode = "qx88";
                }
                if (transaction.PartnerCode == "tn99")
                {
                    requestData.PartnerCode = "tn99";
                }
                var url = urlBaseServiceBank;
                    ...
                    if (resObj.ResponseCode > 0)
                    {
                        ...
                        if (add > 0)
                        {
                            ...
                            _ = Task.Run(async () =>
                            {
                                try
                                {
                                    await LogInfo(bankinfo);
                                }
                                catch (Exception ex)
                                {
                                    NLogLogger.Info("loginfo ex" + ex.Message);
                                }
                            });
                            //MDrumBankLib.LogBankInfo(bankinfo);
                            ...
                            if (tran.BankName.ToUpper() != "MOMO")
                            {
                                ...
                                //NLogLogger.Info("VietUrl: " + VietUrl);
                                if (transaction.PartnerCode == "nh88" || transaction.PartnerCode == "nh99")
                                {
                                    //VietUrl = HttpUtility.UrlEncode(VietUrl);
                                    VietUrl = $"https://img.vietqr.io/image/{addTran.BankCode}-{bankinfo.BankId}-compact.jpg?amount={orderRes.Amount}&addInfo={HttpUtility.UrlEncode(orderRes.OrderNo)}";
                                }
                                ...
                                if(transaction.PartnerCode=="qx88")
                                {
                                    orderRes.LinkOpenApp = String.Format("https://info.qxapp.info/Pages/Bank.aspx?orderNo={0}", add.ToString());
                                    orderRes.LinkWebView = String.Format("https://info.qxapp.info/Pages/Bank.aspx?orderNo={0}", add.ToString());
                                }
                                if (transaction.PartnerCode == "tn99")
                                {
                                    orderRes.LinkOpenApp = String.Format("https://info.tn99app.info/Pages/Bank.aspx?orderNo={0}", add.ToString());
                                    orderRes.LinkWebView = String.Format("https://info.tn99app.info/Pages/Bank.aspx?orderNo={0}", add.ToString());
                                }
                                orderRes.QRCode = VietUrl;
                                orderRes.QRCodeBase64 = bankinfo.QRCodeBase64;
                                orderRes.Url = VietUrl;
                                ...
                                //orderRes.QRCodeBase64 = QRCodeBase64;
                                //orderRes.QRCodeBase64 = "";

                            }
                            if (transaction.PartnerCode == "go99" || transaction.PartnerCode == "nohu888" || transaction.PartnerCode == "hn002" || transaction.PartnerCode.Contains("shdsn"))
                            {
                                orderRes.QRCodeBase64 = "";
                                orderRes.QRCode = "";
                                orderRes.LinkOpenApp = "";
                            ...
                            //if (transaction.PartnerCode == "go99" || transaction.PartnerCode == "nohu888" || transaction.PartnerCode == "hn002" || transaction.PartnerCode.Contains("shdsn"))
                            //{
                            //    NLogLogger.Info(new string[] { "API", "Respone", jsonContent });
                            //}

                            return new APIResponse((int)ResponseCode.TransactionSuccessful)
                            {
                                ResponseContent = jsonContent.Replace("\\u0026", "&")
                            };
                        }
                        else
                        {
        ...
        public APIResponse CallbackV3(MDrumBankLib.CallbackV3 callback, string partnerCode)
        {
            ...
            if (callback.CheckTransId != "1")
            {
                ...
                if (partnerCode == "sn2")
                {
                    ...
                    {
                        TelegramNotify.SendTeleV2("-4882076202", "[SN02] có lệnh nạp lớn " + callback.Amount.ToString("#,#").Replace(",", ".") + " " + callback.NoteFull + "=> cần kiểm tra trước khi callback ");
                        return apiResponse;
                    }
                    if (!string.IsNullOrEmpty(MDrumBankLib.GetBankSuccess3(keyBank)))
                    {
                        TelegramNotify.SendTeleV2("-4882076202", "[SN02] có lệnh nạp trùng nội dung " + callback.Amount.ToString("#,#").Replace(",", ".") + " " + callback.NoteFull + "=> cần kiểm tra trước khi callback ");
                        return apiResponse;
                    }
                }
        ...
        public APIResponse CallbackV2(MDrumBankLib.Callback callback, string partnerCode)
        {
            ...
            //    {
            //        //elegramNotify.SendTeleV2("-4882076202", "[SN02] có lệnh nạp lớn " + callback.Amount.ToString("#,#").Replace(",", ".") + " " + callback.NoteFull + "=> cần kiểm tra trước khi callback ");
            //        return apiResponse;
            //    }
        ...
        public APIResponse Callback(MDrumBankLib.Callback callback)
        {
            ...
            if (order == null)
            {
                ...
                if (callback.CheckTransId != "1" && !callback.NoteFull.Contains("SYS"))
                {
                    if (!string.IsNullOrEmpty(callback.BankTransId) && callback.Note.Length == 10)
                    {
                        new BankTransaction().UpdateCodeById(callback.TransId, "[ORDER_NOTFOUND]|" + callback.Note);
                    }
                }
                return apiResponse;
            }
            if (order.Status == 1)
            {
                ...
                if (callback.CheckTransId != "1" && !order.LogContent.Contains("update by"))
                {
                    if (!string.IsNullOrEmpty(callback.BankTransId) && callback.Note.Length == 10)
                    {
                        new BankTransaction().UpdateCodeById(callback.TransId, "[ORDER_DUPLICATE]|" + callback.Note);
                    }
                }
                return apiResponse;
            }
            if (callback.CheckTransId != "1" && order.PartnerCode == "5pay")
            {
                if (order.Status == 0)
                {
                    if (!string.IsNullOrEmpty(callback.BankTransId))
                    {
                        ...
                        {
                            var partner2 = new Partners().GetCache(order.PartnerCode);
                            if (partner2.SMSUrl == "5pay")
                            {
                                if (!string.IsNullOrEmpty(callback.BankTransId) && callback.Note.Length == 10)
                                {
            ...
            if (callback.Amount > 10000000)
            {
                ...
                {
                    TelegramNotify.SendTeleV2("-4775802643", "Có lệnh nạp momo từ đối tác " + order.PartnerCode + ", số tiền " + callback.Amount.ToString("#,#").Replace(",", "."));
                }
                else
                {
                    TelegramNotify.SendTeleV2("-4775802643", "Có lệnh nạp bank từ đối tác " + order.PartnerCode + ", số tiền " + callback.Amount.ToString("#,#").Replace(",", "."));
                }
            }

            if (order.PartnerCode == "123b" || order.PartnerCode == "sn33" || order.PartnerCode == "b23")
            {
                ...
                if (user.Balance >= 500000000)
                {
                    ...
                    {
                        TelegramNotify.SendTeleFast("-5138534907", "Số dư  " + order.PartnerCode + " hơn 500tr rồi, các anh rút giúp em nhé");
            ...
            if (order.Status != (int)ResponseCode.TransactionSuccessful)
            {
                ...
                order.Reward = Convert.ToInt64(callback.Amount * rw2);


                order.Fee = Convert.ToInt64(callback.Amount * ck);
                ...
                //    {
                //        TelegramNotify.SendTeleV2("-4882076202", "[Chuyển thường] có lệnh chuyển thường " + order.PartnerCode + " " + callback.Amount.ToString("#,#").Replace(",", ".") + " " + callback.Note + "=> báo lại khách hàng ");
                //    }
                //}
                ...
                if (resultupdate >= 0)
                {
                    ...
                    if (!string.IsNullOrEmpty(partner.SMSCommand))
                    {
                        ...
                        if (rw > 0)
                        {
                            var TotalR = Convert.ToInt64(callback.Amount * rw);
