...
public class Telegram : IHttpHandler
{
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (resObj.message.text.StartsWith("/bill"))
                    {
                        ...
                        if (trans.Status >= 1)
                        {
                            var url = String.Format("http://139.180.147.57:1587/DetailScreen6.aspx?orderNo={0}&refcode={1}&chatid={2}", trans.TransactionID, trans.RefCode, resObj.message.chat.id.ToString());
                            SendBill(url);
    ...
    {
        string BotToken = "8351377347:AAGUeBCSu7PwC65mVMHAsSDt8no-poAKqvo";
        string url =
    ...
    public static void SendTele(string id, string message)
    {
        ...
        try
        {
            ...
            var requestUrl = string.Format("https://api.telegram.org/bot8351377347:AAGUeBCSu7PwC65mVMHAsSDt8no-poAKqvo/sendMessage?chat_id={1}&text={0}", message, id);
            var webclient = new WebClient();
