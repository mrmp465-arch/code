            ...
            try
            {
                ...
                else
                {
                    ...
                    if (resObj.message.text.StartsWith("/bill"))
                    {
                        ...
                        if (trans.Status >= 1)
                        {
                            var url = String.Format("http://149.28.151.96:1468/DetailScreen6.aspx?orderNo={0}&refcode={1}&chatid={2}", trans.TransactionID, trans.RefCode, resObj.message.chat.id.ToString());
                            SendBill(url);
