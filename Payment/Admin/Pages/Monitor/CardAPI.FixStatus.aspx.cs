...
public partial class Pages_Monitor_CardAPI_FixStatus : System.Web.UI.Page
{
    ...
    protected void btUpdate_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            Partners _Partner = new Partners().GetCache(_CardAPILog.PartnerCode);
            int cbStatus = 0;
            if (_CardAPILog.Status == 1 || _CardAPILog.Status == 2)
            {
                ...
                if (oldStatus < 1)
                {
                    ...
                    {
                        var rw = GetRW(_Partner.PartnerCode, _CardAPILog.CardType);
                        if (rw > 0)
                        {
                            var TotalR = Convert.ToInt64(Amount * rw);
                            var Balancedesc2 = String.Format("Cộng tiền hoa hồng nạp thẻ đối tác {4} số tiền: {3} mgd: {0}-{1}-{2}", _CardAPILog.TransactionID, _CardAPILog.CardType, _CardAPILog.CardSerial + "-" + _CardAPILog.CardCode, Convert.ToInt64(Amount).ToString("#,#").Replace(",", "."), _Partner.PartnerCode);
                            UpdatePartnerBalanceReward(_Partner.SMSCommand, TotalR, Balancedesc2, "RCardIn_" + _CardAPILog.TransactionID.ToString());
                        }
                    }
                }
