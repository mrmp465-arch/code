    ...
    protected void lnCallback_Click(object sender, EventArgs e)
    {
        ...
        if (!string.IsNullOrEmpty(url))
        {
            ...
            if (_CardAPILog.PartnerCode == "sn1")
            {
                ...
                Task.Run(() => CallbackJson(url, serializer.Serialize(datacb2), _CardAPILog.TransactionID).ConfigureAwait(false));
            }
            else
            {
                ...
                Task.Run(() => CallbackJson(url, serializer.Serialize(apiResponse), _CardAPILog.TransactionID).ConfigureAwait(false));
            }
