<%@ Page Title="" Language="C#" MasterPageFile="~/Layout/Layout.master" AutoEventWireup="true" CodeFile="BuyCard.Report.DoiSoat.aspx.cs" Inherits="Pages_Monitor_BuyCard_Report_DoiSoat" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <section class="content">
        <div class="nav-tabs-custom">
            ...
            <div class="tab-content no-padding">
                <div class="chart tab-pane <% if (Type == "1" || Type == null)
                    ...
                    <div class="box-body " style="clear: both; overflow-x: SCROLL">
                        <table class="table table-bordered" id="TableResponsive1" style="margin-top: 20px; clear: both;">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>Partner</th>
                            ...
                            <tbody>
                                <%
                                    ...
                                    if (lstReportDoiSoat1 != null)
                                    {
                                        ...
                                        foreach (var item in lstReportDoiSoat1)
                                        {
                                            ...
                                            {
                                                count2 = lstReportDoiSoat1.Count(e => e.Provider == item.Provider && e.PartnerID == item.PartnerID);
                                ...
                                <tr>
                                    ...
                                    <%} %>
                                    <%if (count2 > 0)
                                    { %><td rowspan="<%=count2 %>"><%= item.Provider %></td>
                                    <%} %>
                                    <%--<td ><%= item.Provider %></td>--%>
                                    <td class="right"><%= Convert.ToInt64(item.Amount).ToString("N0") %></td>
                                    <td class="right"><%= item.Quantity.ToString("N0") 
                                        
                                    %></td>
                                    <td class="right"><%= Convert.ToInt64((item.Quantity*item.Amount)).ToString("N0") %></td>
                                </tr>
                                ...
                                <%
                                    ...
                                    if (lstReportDoiSoat2 != null)
                                    {
                                        ...
                                        foreach (var item in lstReportDoiSoat2)
                                        {
                                            ...
                                            if (Provider != item.Provider)
                                            {
                                                count2 = lstReportDoiSoat2.Count(e => e.Provider == item.Provider && e.ProviderCode == item.ProviderCode);
                                                Provider = item.Provider;
                                            }
                                %>
                                <tr>
      ...
      { %><td rowspan="<%=count2 %>"><%= item.Provider %></td>
                                    ...
                                    <%--<td ><%= item.Provider %></td>--%>
                                    <td class="right"><%= Convert.ToInt64(item.Amount).ToString("N0") %></td>
                                    <td class="right"><%= item.Quantity.ToString("N0") %></td>
                                    <td class="right"><%= Convert.ToInt64((item.Quantity*item.Amount)).ToString("N0") %></td>
                                </tr>
                                <% Total2 += item.Quantity * item.Amount;
                                        }
                                    } %>
                                <tr class="Total">
                                    <td colspan="5" style="text-align: center;"><b>Total</b></td>
                    ...
                    <div class="box-footer">
                        <input type="button" class="btn btn-info pull-right" value="export Excel" onclick="exportThisWithParameter('TableResponsive2', 'DoiSoatThe')" />
                    </div>
                </div>
                <%} %>
            </div>
