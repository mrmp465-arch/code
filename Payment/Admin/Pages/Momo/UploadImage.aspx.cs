...
using System.Data;
using System.Linq;
public partial class Pages_Momo_UploadImage : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        AppUtils.CheckRoles("pages/bankewalletservice/momo/uploadimage.aspx");
    ...
    private void init()
    {
        ...
        if (_bank.StatusDetection == 1)
        {
            lbDetech.Text = "Đã Detech";
        }
        if (_bank.StatusDetection == 0)
        {
        ...
        lblAccount.Text = " Tài khoản  : " + _bank.MomoId;

        var bankCode = "MOMO";
        var bankId = _bank.MomoId;
    ...
    protected void btAdd_Click(object sender, EventArgs e)
    {
        ...
        var lstProfileImage2 = new List<MomoProfileImage>();
        lstProfileImage2.Add(new MomoProfileImage
        {
            Base64 = "/cmspay/content/noimage.png",
            Detection = "",
            ImgName = ""
        ...
        if (!string.IsNullOrEmpty(_bank.ProfileImage))
        {
            try
            {
                lstProfileImage2 = serializer.Deserialize<List<MomoProfileImage>>(_bank.ProfileImage);
        ...
        for (var i = 0; i < 8; i++)
        {
            ...
            if (!string.IsNullOrEmpty(lstProfileImage[i].ImgName))
            {
                lstProfileImage2[i].Base64 = lstProfileImage[i].Base64;
                lstProfileImage2[i].ImgName = lstProfileImage[i].ImgName;
            }
            else
            {
                var idex = i + 1;
                string filePath = Path.Combine(UploadFolderPhysical, idex + ".jpg");
                if (File.Exists(filePath))
                {
                    lstProfileImage2[i].Base64 = "";
                    lstProfileImage2[i].ImgName = idex + ".jpg";
                }
                else
                {
                    lstProfileImage2[i].Base64 = "/cmspay/content/noimage.png";
                    lstProfileImage2[i].ImgName = "";
                }
            }

        }

        _bank.ProfileImage = serializer.Serialize(lstProfileImage2);
        if (chkIsActive.Checked)
        {
            _bank.StatusDetection = 0;
        }
        _bank.Update();
        //_bank.ProfileImage = serializer.Serialize(lstProfileImage);
        ...
        var _userLog = new UserLog
        {
            ...
            Description = "Cập nhật momo |" + _bank.MomoId
        };
        _userLog.Add();
        Response.Redirect(Constant.ADMIN_PATH + Resources.Url.MomoAccount);
    }
    ...
    private string FileToBase64ImageUrl2(string filePath)
    {
        if (!File.Exists(filePath))
            return "/cmspay/content/noimage.png";
