...
public class Telegram : IHttpHandler
{
                ...
                else
                {
                    ...
                    if (resObj.message.text.StartsWith("/in"))
                    {
                        ...
                        if (trans == null)
                        {
                            ...
                            SendTeleV4(resObj.message.chat.id.ToString(), "Không tìm thấy đơn | Transaction not found | 没有找到应用程序", message_id);
                            return;
                        }
                        if (trans.Status >= 1)
                        {
                        ...
                        else
                        {
                            ...
                            SendTeleV4(resObj.message.chat.id.ToString(), "Giao dịch đang chờ xử lý | Transaction processing | 交易待处理", message_id);
                            return;
                        }
                    ...
                    if (resObj.message.text.StartsWith("/bill"))
                    {
                        ...
                        //    //};
                        //    var url = String.Format("http://149.28.151.96:1468/DetailScreen.aspx?orderNo={0}&refcode={1}&chatid={2}", checkorder.TransactionID, checkorder.RefCode, resObj.message.chat.id.ToString());
                        //    SendBill(url);
                        ...
                        if (trans.Status >= 1)
                        {
                            if (!string.IsNullOrEmpty(trans.Mobile))
                            {
                                ...
                                var url = String.Format("http://149.28.151.96:1468/DetailScreen10.aspx?orderNo={0}&refcode={1}&chatid={2}&message_id={3}", trans.TransactionID, trans.RefCode, resObj.message.chat.id.ToString(), message_id);
                                SendBill(url);
                            }
                            //SendTeleV2(resObj.message.chat.id.ToString(), "Transaction success");
                            return;
                        ...
                        else
                        {
                            if (trans.Status == -1)
                            {
                                if ((trans.LogContent.Contains("không trùng ") || trans.LogContent.Contains("tìm thấy")))
                                {
                                    ...
                                    {
                                        SendTeleV4(resObj.message.chat.id.ToString(), "Ngân hàng nhận tiền lỗi | The bank accepts the money for the error | 银行接受了因错误造成的损失。 ", message_id);
                                        return;
                                    }
                                    else
                                    {
                                        SendTeleV4(resObj.message.chat.id.ToString(), "Tài khoản nhận tiền không trùng khớp | The receiving account does not match | 收款账户不匹配。", message_id);
                                        return;
                                    }
                                }
                                SendTeleV4(resObj.message.chat.id.ToString(), "Lệnh out thất bại do ngân hàng lỗi | The  order failed due to a bank error |由於銀行故障，退出指令失敗。", message_id);
                                return;
                            }
                            if (trans.Status == -357)
                            {
                            ...
                            SendTeleV4(resObj.message.chat.id.ToString(), "Lệnh đang được xử lý | The order is being processed.| 订单正在处理中。", message_id);
                            return;
                        }

                    }
                    //if (resObj.message.text.StartsWith("/check"))
                    //{
                    //    var refcode = resObj.message.text.Replace("/check", "").Trim();
                    //    var trans = new UserTransaction().GetByRefcode(refcode, Partnecode);
                    //    if (trans != null)
                    //    {
                    //        SendTeleV2(resObj.message.chat.id.ToString(), String.Format("{0} | {1} | {2} |{3} |{4} | {5} ", trans.RefCode,
                    //        GetCommaMark(trans.Type, trans.Note) + trans.Amount.ToString("N0").Replace(".", ","), trans.Note, trans.CreatedTime.ToString("dd/MM/yy HH:mm:ss"),
                    //        trans.BalanceBefore.ToString("N0").Replace(".", ","), trans.Balance.ToString("N0").Replace(".", ",")));
                    //    }
                    //}
                    if (resObj.message.text.StartsWith("/out"))
                    {
                        ...
                        {
                            //return new APIResponse((int)ResponseCode.TransactionNotExists);
                            SendTeleV4(resObj.message.chat.id.ToString(), "Lệnh out không tìm thấy | Transaction Not Found | 未找到 out 命令 ", message_id);
                            return;
                        }
                        if (trans.Status >= 1)
                        {
                        ...
                        else
                        {
                            if (trans.Status == -1)
                            {
                                if (trans.LogContent.Contains("Lỗi truy vấn"))
                                {
                                    ...
                                    SendTeleV4(resObj.message.chat.id.ToString(), "Tài khoản nhận tiền không trùng khớp hoặc không đúng| The receiving account does not match | 收款账户不匹配。 =>" + logmess, message_id);
                                    return;
                                }
                                if (trans.LogContent.Contains("Không tìm thấy"))
                                {
                                    var logmess = GetErrorMessage(trans.LogContent, "Không tìm thấy");
                                    SendTeleV4(resObj.message.chat.id.ToString(), "Tài khoản nhận tiền lỗi| Recipient account error |收款账户错误 =>" + logmess, message_id);
                                    return;
                                }
                                SendTeleV4(resObj.message.chat.id.ToString(), "Lệnh out thất bại do ngân hàng lỗi | The  order failed due to a bank error |由於銀行故障，退出指令失敗。", message_id);
                                return;
    ...
    public static string GetCommaMark(int type, string note)
    {
    ...
    public static void SendTele(string id, string message)
    {
        ...
        try
        {
            ...
            var requestUrl = string.Format("https://api.telegram.org/bot8863680048:AAFM5JHb_6au3CAlwXX38XlZv9OXn8xLlWg/sendMessage?chat_id={1}&text={0}", message, id);
            var webclient = new WebClient();
    ...
    public static void SendTeleV3(string id, string message, string replyToMessageId)
    {
        ...
        try
        {
            ...
            var requestUrl = string.Format("https://api.telegram.org/bot8863680048:AAFM5JHb_6au3CAlwXX38XlZv9OXn8xLlWg/sendMessage?chat_id={1}&reply_to_message_id={2}&text={0}", message, id, replyToMessageId);
            var webclient = new WebClient();
    ...
    {
        string BotToken = "8863680048:AAFM5JHb_6au3CAlwXX38XlZv9OXn8xLlWg";
        string url =
