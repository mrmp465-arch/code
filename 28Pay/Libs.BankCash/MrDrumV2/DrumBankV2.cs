        ...
        public APIResponse Cash(APITransaction transaction)
        {
            ...
            else
            {
                ...
                if (!lstbankcode.Exists(x => x.code.ToUpper() == request.BankName.ToUpper()))
                {
                    if (string.IsNullOrEmpty(chatid))
                    {
                        TelegramNotify.SendTeleV2("-1003939306637", "[" + transaction.PartnerCode + "]: lệnh rút  " + request.RefCode + " " + request.BankName + " => sai mã ngân hàng");
                    }
                    else
                    {
                ...
                else
                {
                    if (lstbankcode.Exists(x => x.code.ToUpper() == request.BankName.ToUpper() && x.isTransfer == 0))
                    {
                        if (string.IsNullOrEmpty(chatid))
                        {
                            TelegramNotify.SendTeleV2("-1003939306637", "[" + transaction.PartnerCode + "]: lệnh rút  " + request.RefCode + " " + request.BankName + "=> mã ngân hàng nhận bảo trì ");
                        }
                        else
                        {
            ...
            if (!string.IsNullOrEmpty(partner.Hotline))
            {
                ...
                if (user.Balance < request.Amount)
                {
                    if (string.IsNullOrEmpty(chatid))
                    {
                        TelegramNotify.SendTeleV2("-1003939306637", "Đối tác " + transaction.PartnerCode + "  không đủ số dư để tạo lệnh bank out - " + request.RefCode);
                    }
                    else
                    {
            ...
            try
            {
                ...
                if (resultdeduct < 0)
                {
                    TelegramNotify.SendTeleV2("-1003939306637", "Đối tác " + transaction.PartnerCode + "  không đủ số dư để tạo lệnh bankout");
                ...
                //        if (Balance < 20000000)
                //        {
                //            TelegramNotify.SendTeleV2("-1003902216204", "Sô dư bank out dưới 20tr => anh em bơm tiền vào nhé  !!!! ");
                ...
                {
                    TelegramNotify.SendTeleV2("-1003902216204", "[Duyệt] Có lệnh bankout cần duyệt  " + request.Amount.ToString("#,#").Replace(",", ".") + " từ đối tác " + transaction.PartnerCode + " RefCode " + request.RefCode);
                ...
                if (request.Type == "momocash")
                {
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            //{
                            //    TelegramNotify.SendTeleV2("-1003939306637", "MOMOOUT :" + resObj.Description);
                            //}
                            var log = new LogInfo
                            {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            if (resObj.Description.Contains("busy") || resObj.Description.Contains("đủ điều kiện"))
                            {
                                ...
                                {
                                    TelegramNotify.SendTeleV2("-1003902216204", "Tài khoản bank out không đủ tiền=> Vui lòng duyệt tay lệnh này");
                                }
                            ...
                            if (resObj.Description.ToLower().Contains("system") || resObj.Description.ToLower().Contains("fail"))
                            {
                                TelegramNotify.SendTeleV2("-1003939306637", "BANKOUT :" + resObj.Description);
                            }
                    ...
                    catch
                    {
                        ...
                        TelegramNotify.SendTeleV2("-1003939306637", "Nghi vấn lỗi TCP !!!!! ");
                        return new APIResponse(1);
                        ////if (resObj.Description.ToLower().Contains("system") || resObj.Description.ToLower().Contains("fail"))
                        ////{
                        ////    TelegramNotify.SendTeleV2("-1003939306637", "BANKOUT :" + resObj.Description);
                ...
                try
                {
                    ...
                    else
                    {
                        if (resObj.Description.Contains("is busy"))
                        {
                            ...
                            TelegramNotify.SendTeleV2("-1003902216204", "Có lệnh bankout cần duyệt  " + _CardAPILog.Amount.ToString("#,#").Replace(",", ".") + " từ đối tác " + _CardAPILog.PartnerCode + " RefCode " + _CardAPILog.RefCode);
        ...
        public APIResponse Callback(DrumBankLib.BankResponse callback)
        {
                ...
                if (order.Status == 0)
                {
                    ...
                    if (!string.IsNullOrEmpty(order.ReturnUrl))
                    {
                        ...
                        if(partner.RequestType>0)
                        {
                            ...
                            {
                                var url = String.Format("http://149.28.155.162:1586/DetailScreen3.aspx?orderNo={0}&refcode={1}&chatid={2}", order.TransactionID, order.RefCode, partner.SMSPlusUrl);
                                SendBill(url);
                            }
                        }    
            ...
            if (callback.ResponseCode < 0)
            {
                ...
                if ((callback.Description.Contains("busy") || callback.Description.Contains("Bank Code Invalid")) && order.ProviderCode == "drumbankcash" && isCancel == 0)
                {
                    ...
                    //                order.Update();
                    //            }
                    //            TelegramNotify.SendTeleV2("-1003939306637", "có giao dịch nghi vấn cần xử lý  RefCode " + order.RefCode + " ,đối tác " + order.PartnerCode);
                    //            return apiResponse;
                    //        }
                    //    }
                    //}
                    ...
                    TelegramNotify.SendTeleV2("-1003902216204", mess);
                    ...
                    try
                    {
                        ...
                        if (check != null)
                        {
                            ...
                            if (check.Description == "Start Operation Failed")
                            {
                                ...
                                //TelegramNotify.SendTeleV2("-1003939306637", "Nghi vấn lỗi Proxy");
                            }
                            else
                            {
                                ...
                                //    TelegramNotify.SendTeleV2("-1003939306637", "Lỗi treo phone client, bật phần mềm trên máy tính kết nối lại");
                                //}
                                if (check.Description.Contains("Device") || check.Description.Contains("OTP") || check.Description.Contains("Start") || check.Description.Contains("Screen") || check.Description.Contains("connected"))
                                {
                                    ...
                                    {
                                        TelegramNotify.SendTeleV2("-1003939306637", "Lỗi thiết bị " + bank.Computer + "-" + bank.PhoneDevice + " : " + check.Description + " => vui lòng kiểm tra lại thiết bị ok mới duyệt");
                                    }
                                    else
                                    {
                                        TelegramNotify.SendTeleV2("-1003902216204", "Lỗi thiết bị " + bank.Computer + "-" + bank.PhoneDevice + " : " + check.Description + " => vui lòng kiểm tra lại thiết bị ok mới duyệt");
                                    }
                        ...
                        try
                        {
                            ...
                            if (check != null)
                            {
                                ...
                                if ((check.Description.Contains("Verify Method") || check.Description.Contains("Submit Method: Null or Empty") || check.Description.Contains("AccountInquiry Null or Empty") || check.Description.Contains("OTP not is numeric")))
                                {
                                    ...
                                    //TelegramNotify.SendTeleV2("-1003902216204", "Lỗi verify => duyệt lại luôn");
                                }
                                else
                                {
                                    if (check.Description.Contains("Device") || check.Description.Contains("OTP") || check.Description.Contains("Start") || check.Description.Contains("Screen") || check.Description.Contains("connected"))
                                    ...
                                    if (check.Status == -51)
                                    {
                                        ...
                                        var bank = new BankAccounts().Get(check.PartnerBankId, check.PartnerBankCode);
                                        System.Threading.Thread.Sleep(500);
                                        TelegramNotify.SendTeleV2("-1003939306637", "Tài khoản " + check.PartnerBankCode + " -" + bank.BankName + " lệch số dư=> động bộ lại trước khi duyệt");
                                    }
                                }
                                ...
                                {
                                    var bank = new BankAccounts().Get(check.PartnerBankId, check.PartnerBankCode);
                                    TelegramNotify.SendTeleV2("-1003939306637", "Nghi vấn khoá " + bank.BankCode + " - " + bank.BankName + " => vui lòng kiểm tra lại ");
                                    bank.Status = -1;
                                    bank.Update();
                                }
                ...
                if (callback.ResponseCode == -2)
                {
                    ...
                    TelegramNotify.SendTeleV2("-1003939306637", "có giao dịch nghi vấn cần xử lý  RefCode " + order.RefCode);
                    return apiResponse;
                }
                ...
                if (order.Status == 0)
                {
                    ...
                    //    if (order.BankCode == "MOMO")
                    //    {
                    //        TelegramNotify.SendTeleV2("-1003939306637", "MOMOOUT :" + callback.Description);
                    //    }
                    //    else
                    //    {
                    //        TelegramNotify.SendTeleV2("-1003939306637", "BANKOUT :" + callback.Description);
                    //    }
                    //}
