...
namespace Libs.BankDirect.MDrumV2
{
    public class MDrumV2Bank : IBankDirectV2Handler
    {
                    ...
                    if (resObj.ResponseCode > 0)
                    {
                        ...
                        if (add > 0)
                        {
                            ...
                            var orderRes = new Order()
                            {
                                ...
                                Timeout = 60 * 10,
                                Url = String.Format("https://info.28app.info/Pages/Momo.aspx?orderNo={0}", add.ToString()),
                                BankCode = tran.BankName,
        ...
        public APIResponse Callback(MDrumBankLib.Callback callback)
        {
            ...
            //set success
            DataCaching.SetCache("LastOrderSuccess", "1", 150);

            if (callback.Amount > 34000000)
            {
                if (order.BankCode.ToUpper() == "MOMO")
                {
                    TelegramNotify.SendTeleV2("-1003939306637", "Có lệnh nạp momo lớn từ đối tác " + order.PartnerCode + ", số tiền " + callback.Amount.ToString("#,#").Replace(",", "."));
                }
                else
                {
                    TelegramNotify.SendTeleV2("-1003939306637", "Có lệnh nạp bank lớn từ đối tác " + order.PartnerCode + ", số tiền " + callback.Amount.ToString("#,#").Replace(",", "."));
                }
            }
            ...
            if (order.Status != (int)ResponseCode.TransactionSuccessful)
            {
                ...
                //    {
                //        TelegramNotify.SendTeleV2("-1003939306637", "[Chuyển thường] có lệnh chuyển thường " + order.PartnerCode + " " + callback.Amount.ToString("#,#").Replace(",", ".") + " " + callback.Note + "=> báo lại khách hàng ");
                //    }
                //}
                ...
                //    else
                //    {
                //        TelegramNotify.SendTeleV2("-1003939306637", "[" + order.PartnerCode + "]: lệnh nạp sai số tiền refcode " + order.RefCode + ", code " + order.OrderNo + " , amount user " + Convert.ToInt64(order.Amount).ToString("#,#").Replace(",", ".") + ", amount " + callback.Amount.ToString("#,#").Replace(",", ".") + " => đã cộng số dư cho merchant, game cộng tay giúp");
                //    }
