<%@ Page Title="" Language="C#" MasterPageFile="~/Layout/Layout.master" AutoEventWireup="true" CodeFile="CardAPI.Report.Profit.aspx.cs" Inherits="Pages_Monitor_CardAPI_Report_Profit" %>
                        ...
                        <table class="table table-striped" id="TableResponsive" style="margin-top: 20px; clear: both;">
                            ...
                            <tbody>
                                <asp:Repeater ID="rptList" runat="server">
                                    <ItemTemplate>
                                        <tr id="tr<%# Container.ItemIndex + 1 %>" data-url="&year=<%#Eval("Time") %>&CardType=<%#Eval("CardType") %>&PartnerCode=<%#Eval("PartnerCode") %>&Provider=<%#Eval("Provider") %>">
                                            <td></td>
                                            <td><%#Eval("Time")%></td>
                                            ...
                                            <td><%#Convert.ToInt64(Eval("Profit")).ToString("N0").Replace(",", ".") %></td>
                                            <td><%# AppUtils.AmountToPercentFit(Eval("TotalAmount").ToString(),Eval("Profit").ToString()) %></td>
                                            <td><%#Eval("CardType") %></td>
                                            <% if (AppUtils.IsAdmin || AppUtils.IsProvider)
                                                {%>
                                            <td><%#Eval("Provider") %></td>
