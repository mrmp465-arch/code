...
public partial class Pages_BankEWalletService_Bank_Account_Edit : System.Web.UI.Page
{
    ...
    private void init()
    {
        ...
        //if (Request.RawUrl.Contains("#balance-info"))
        //    hidBalanceTAB.Value = "#balance-info";
        //Control
        //serializer.MaxJsonLength = int.MaxValue;
        ...
        _bank = _bank.Get();
        //var url = Request.Url.GetLeftPart(UriPartial.Authority) + "/cmspay/pages/bankewalletservice/bank/account.edit.aspx?id=" + AppUtils.Request("id") + "#balance-info";
        //NLogLogger.Info("url " +url);
        ...
        var lstBank4 = new ITBank().GetList().Where(x => x.Type == "PRIVATE" && x.Status == 1).ToList();
        foreach (var item in lstBank4)
        {
            item.BankCode += "-" + item.BankName + "-" + item.BankId;
        }

        drpBankCode4.DataSource = lstBank4;
        drpBankCode4.DataTextField = "BankCode";
        drpBankCode4.DataValueField = "BankCode";
        drpBankCode4.DataBind();

        drpBankCode4.Items.Insert(0, new ListItem("Chọn từ tài khoản riêng", ""));
        ...
        txtNoteForm4.Text = _bank.BankName + " CHUYEN KHOAN";
        ...
        txtNote.Text = _bank.Note;
        ViewState["Solution"] = _bank.Solution;

        if (HttpContext.Current.Session["Amount"] != null)
        {
            if (!string.IsNullOrEmpty(HttpContext.Current.Session["Amount"].ToString()))
            {
                txtAmountForm4.Text = HttpContext.Current.Session["Amount"].ToString();
            }
        }
        ...
        //    string UploadFolderPhysical = Path.Combine(@"Z:\24HPAY", _bank.BankCode, _bank.BankId);
        //    try
        //    {
        //        for(var i = 0; i < 8; i++)
        //        {
        //            SaveUploadImageBase(lstProfileImage[i].Base64, UploadFolderPhysical, i+1);
        //        }    
    ...
    protected void btnCashForm4_Click(object sender, EventArgs e)
    {
        ...
        if (resObj != null)
        {
            ...
            if (resObj.ResponseCode == (int)ResponseCode.TransactionSuccessful)
            {
                ...
                HttpContext.Current.Session["Amount"] = txtAmountForm4.Text;
                //resTextForm4.InnerHtml = String.Format("<i>Hệ thống đã nhận lệnh thành công. Bạn vui lòng xem chi tiết kết quả tại Bank Log</i>  <script>setTimeout(function() { window.location.href = '{0}'; }, 500);</script>)",url);
                ...
                TelegramClient.SendTeleV2("-5163926506", "Chuyển tiền bank " + drpBankCode.SelectedValue + " | " + txtBankId.Text + " | " + txtAmountForm4.Text + " |" + drpBankCodeForm4.SelectedValue + " | " + txtBankIdForm4.Text + " từ user " + AppUtils.UserName);
                _userLog.Add();

                resTextForm4.InnerHtml = String.Format(@"
                <i>Hệ thống đã nhận lệnh thành công. Bạn vui lòng xem chi tiết kết quả tại Bank Log</i>
                <script>
                    setTimeout(function () {{
