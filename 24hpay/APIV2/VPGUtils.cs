    ...
    public class VPGUtils
    {
        public static string Request(string partnerCode, string serviceCode, string commandCode, string requestContent, string signature)
        {
            ...
            try
            {
                ...
                if (_APIResponse.ResponseCode == (int)ResponseCode.ProviderNotFound|| _APIResponse.ResponseCode == (int)ResponseCode.SystemMaintain)
                {
        ...
        public static string RequestV2(string partnerCode, string cardCode, string cardSeri, int amountUser, string cardType,  string refCode, string url, string signature)
        {
            JavaScriptSerializer serializer = new JavaScriptSerializer();
            // Ghi log giao dịch
            ...
            try
            {
                var jsonContent = $"\"CardSerial\":\"{cardSeri}\",\"CardCode\":\"{cardCode}\",\"CardType\":\"{cardType}\",\"AccountName\":\"{refCode}\",\"AppCode\":\"\",\"RefCode\":\"{refCode}\",\"Amount\":0,\"AmountUser\":{amountUser},\"CallbackUrl\":\"{url}\"";
                _APITransaction.PartnerCode = partnerCode;
                _APITransaction.ServiceCode = serviceCode;
                _APITransaction.CommandCode = commandCode;
