...
namespace Libs.CardTelco.EZPay
{
    public class EZPayCard : ICardTelcoHandler
    {

        protected string providerName = string.Empty;
        string serviceUrl = "http://127.0.0.1:1587/";
        ...
        public APIResponse UseCard(APITransaction transaction)
        {
            ...
            try
            {
                ...
                NLogLogger.Info(new string[] { "EZPayCard", transaction.TransactionID.ToString(), "TopupRequest", serializer.Serialize(topupRequest) });
                TopupResponse cardResult = ProcessCard(topupRequest);
                NLogLogger.Info(new string[] { "EZPayCard", transaction.TransactionID.ToString(), "TopupResponse", serializer.Serialize(cardResult) });
            ...
            catch (Exception ex)
            {
                ...
                {
                    case 1:
                        NLogLogger.Info(new string[] { "EZPayCard", transaction.TransactionID.ToString(), "Error", "Step1", ex.Message.Replace("\n", " ") });
                        _APIResponse = new APIResponse((int)ResponseCode.RequestContentInvalid);
                        break;
                    case 2:
                        NLogLogger.Info(new string[] { "EZPayCard", transaction.TransactionID.ToString(), "Error", "Step2", ex.Message.Replace("\n", " ") });
                        _APIResponse = new APIResponse((int)ResponseCode.SystemError);
                        break;
                    case 3:
                        NLogLogger.Info(new string[] { "EZPayCard", transaction.TransactionID.ToString(), "Error", "UseCard", ex.Message.Replace("\n", " ") });
                        ...
                        break;
                    case 4:
                        NLogLogger.Info(new string[] { "EZPayCard", transaction.TransactionID.ToString(), "Error", "Step4", ex.Message.Replace("\n", " ") });
        ...
        {
            JavaScriptSerializer serializer = new JavaScriptSerializer();
            //NLogLogger.Info(new string[] { "EZPayCard", "TopupRequest", serializer.Serialize(topupRequest) });
            ...
            try
            {
                ...
                return serializer.Deserialize<EZPay.TopupResponse>(serviceResponse);
            }
            catch (Exception e)
            {
