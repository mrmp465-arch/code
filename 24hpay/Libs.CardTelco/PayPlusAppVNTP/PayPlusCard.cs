...
namespace Libs.CardTelco.PayPlusAppVNTP
{
    public class PayPlusCard : ICardTelcoHandler
    {
        ...
        private string serviceUrl = ConfigurationManager.AppSettings["AppVNTPService"] ?? "http://127.0.0.1:1584/";
        ...
        public APIResponse UseCard(APITransaction transaction)
        {
            ...
            try
            {
                ...
                if (_CardAPILog.ReturnValue < 0)
                {
                    ...
                    {
                        NLogLogger.Info(new string[] { "VNTPAppCardTopup", transaction.TransactionID.ToString(), transaction.PartnerCode, "TopupRequest", request.CardSerial, request.CardCode, "Error Insert Data" });
                ...
                NLogLogger.Info(new string[] { "VNTPAppCardTopup", transaction.TransactionID.ToString(), "VNTPAppCardTopupRequest", serializer.Serialize(topupRequest) });
                var tryAgain = 0;
                TopupResponse cardResult = ProcessCard(topupRequest);
                //while (cardResult.code == (int)ResponseCode.ServiceIsLocked && tryAgain < 2)
                //{
                ...
                while ((cardResult.code == (int)ResponseCode.ServiceIsLocked 
                        || cardResult.code == (int)ResponseCode.TransactionLimit
                        || cardResult.code == (int)ResponseCode.SimNotActive) 
                       && tryAgain < 2)
                {
                ...
                NLogLogger.Info(new string[] { "VNTPAppCardTopup", transaction.TransactionID.ToString(), "VNTPAppCardTopupResponse", serializer.Serialize(cardResult) });
            ...
            catch (Exception ex)
            {
                ...
                {
                    case 1:
                        NLogLogger.Info(new string[] { "VNTPAppCardTopup", transaction.TransactionID.ToString(), "Error", "Step1", ex.Message.Replace("\n", " ") });
                        _APIResponse = new APIResponse((int)ResponseCode.RequestContentInvalid);
                        break;
                    case 2:
                        NLogLogger.Info(new string[] { "VNTPAppCardTopup", transaction.TransactionID.ToString(), "Error", "Step2", ex.Message.Replace("\n", " ") });
                        _APIResponse = new APIResponse((int)ResponseCode.SystemError);
                        break;
                    case 3:
                        NLogLogger.Info(new string[] { "VNTPAppCardTopup", transaction.TransactionID.ToString(), "Error", "UseCard", ex.Message.Replace("\n", " ") });
                        _CardAPILog.Description = "Error - UseCard " + ex.Message.Replace("\n", " ");
                        _APIResponse = new APIResponse((int)ResponseCode.TransactionSuspicious);
                        break;
                    case 4:
                        NLogLogger.Info(new string[] { "VNTPAppCardTopup", transaction.TransactionID.ToString(), "Error", "Step4", ex.Message.Replace("\n", " ") });
        ...
        {
            JavaScriptSerializer serializer = new JavaScriptSerializer();
            NLogLogger.Info(new string[] { "VNTPAppCardTopup", "VNTPAppCardTopupRequest", serializer.Serialize(topupRequest) });
            TopupWebMobile _VPGService = new TopupWebMobile(serviceUrl + "TopupWeb.asmx");
            ...
            {
                NLogLogger.Info(new string[] { "VNTPAppCardTopup", "VNTPAppCardTopupRequest", "TimeoutException", topupRequest.transactionId.ToString(), e.Message });
                Thread.ResetAbort();
            ...
            {
                NLogLogger.Info(new string[] { "VNTPAppCardTopup", "VNTPAppCardTopupRequest", "ThreadAbortException", topupRequest.transactionId.ToString(), e.Message });
                Thread.ResetAbort();
                return new TopupResponse()
                {
                    code = (int)ResponseCode.TransactionTimeout,
            ...
            {
                NLogLogger.Info(new string[] { "VNTPAppCardTopup", "VNTPAppCardTopupRequest", "Exception", topupRequest.transactionId.ToString(), e.Message });
                return new TopupResponse()
                {
                    code = (int)ResponseCode.TransactionTimeout,
            ...
            try
            {
                NLogLogger.Info(new string[] { "VNTPAppCardTopup", "VNTPAppCardTopupRequest", topupRequest.transactionId.ToString(), serviceResponse });
                return serializer.Deserialize<TopupResponse>(serviceResponse);
            }
            catch (Exception e)
            {
                NLogLogger.Info(new string[] { "VNTPAppCardTopup", "VNTPAppCardTopupRequest", topupRequest.transactionId.ToString(), e.Message });
                return new TopupResponse()
                {
                    code = (int)ResponseCode.TransactionTimeout,
