                    ...
                    if (resObj.ResponseCode > 0)
                    {
                        ...
                        if (add > 0)
                        {
                            ...
                            var orderRes = new Order()
                            {
                                ...
                                Timeout = 60 * 10,
                                Url = String.Format("https://info.cd99app.info/Pages/Momo.aspx?orderNo={0}", add.ToString()),
                                BankCode = tran.BankName,
                            ...
                            if (tran.BankName.ToUpper() != "MOMO")
                            {
                                ...
                                orderRes.Url = String.Format("https://info.cd99app.info/Pages/Bank.aspx?orderNo={0}", add.ToString());
        ...
        public APIResponse Callback(MDrumBankLib.Callback callback)
        {
            ...
            //{
                if (order.Status == 0)
                {
                    ...
                    {
                        if (Convert.ToInt64(order.Amount) != callback.Amount && order.PartnerCode!="fastpay" )
                        {
                            if (!string.IsNullOrEmpty(callback.BankTransId) )
                            {
            ...
            if (callback.Amount > 34000000)
            {
                ...
                {
                    TelegramNotify.SendTeleV2("-5375462347", "Có lệnh nạp momo lớn từ đối tác " + order.PartnerCode + ", số tiền " + callback.Amount.ToString("#,#").Replace(",", "."));
                }
                else
                {
                    TelegramNotify.SendTeleV2("-5375462347", "Có lệnh nạp bank lớn từ đối tác " + order.PartnerCode + ", số tiền " + callback.Amount.ToString("#,#").Replace(",", "."));
                }
            }
            ...
            if (order.Status != (int)ResponseCode.TransactionSuccessful)
            {
                ...
                //    {
                //        TelegramNotify.SendTeleV2("-5375462347", "[Chuyển thường] có lệnh chuyển thường " + order.PartnerCode + " " + callback.Amount.ToString("#,#").Replace(",", ".") + " " + callback.Note + "=> báo lại khách hàng ");
                //    }
                //}
                ...
                //    else
                //    {
                //        TelegramNotify.SendTeleV2("-5375462347", "[" + order.PartnerCode + "]: lệnh nạp sai số tiền refcode " + order.RefCode + ", code " + order.OrderNo + " , amount user " + Convert.ToInt64(order.Amount).ToString("#,#").Replace(",", ".") + ", amount " + callback.Amount.ToString("#,#").Replace(",", ".") + " => đã cộng số dư cho merchant, game cộng tay giúp");
                //    }
