...
public class Telegram : IHttpHandler
{
                    ...
                    if (resObj.message.text.StartsWith("/bill"))
                    {
                        ...
                        if (trans.Status >= 1)
                        {
                            if (!string.IsNullOrEmpty(trans.Mobile))
                            {
                                ...
                                var url = String.Format("http://149.28.151.96:1468/DetailScreen8.aspx?orderNo={0}&refcode={1}&chatid={2}&message_id={3}", trans.TransactionID, trans.RefCode, resObj.message.chat.id.ToString(), message_id);
                                SendBill(url);
                            }
                            //SendTeleV2(resObj.message.chat.id.ToString(), "Transaction success");
                            return;
    ...
    public static void SendTele(string id, string message)
    {
        ...
        try
        {
            ...
            var requestUrl = string.Format("https://api.telegram.org/bot8870375130:AAHgU08FL8C4Ppt3fN0U2hmIiXk7OrHcG0c/sendMessage?chat_id={1}&text={0}", message, id);
            var webclient = new WebClient();
    ...
    public static void SendTeleV3(string id, string message, string replyToMessageId)
    {
        ...
        try
        {
            ...
            var requestUrl = string.Format("https://api.telegram.org/bot8870375130:AAHgU08FL8C4Ppt3fN0U2hmIiXk7OrHcG0c/sendMessage?chat_id={1}&reply_to_message_id={2}&text={0}", message, id, replyToMessageId);
            var webclient = new WebClient();
    ...
    {
        string BotToken = "8870375130:AAHgU08FL8C4Ppt3fN0U2hmIiXk7OrHcG0c";
        string url =
