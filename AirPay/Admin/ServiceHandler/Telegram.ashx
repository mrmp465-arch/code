                    ...
                    if (resObj.message.text.StartsWith("/bill"))
                    {
                        ...
                        if (trans.Status >= 1)
                        {
                            if (!string.IsNullOrEmpty(trans.Mobile))
                            {
                                ...
                                var url = String.Format("http://139.180.147.57:1587/DetailScreen8.aspx?orderNo={0}&refcode={1}&chatid={2}&message_id={3}", trans.TransactionID, trans.RefCode, resObj.message.chat.id.ToString(), message_id);
                                SendBill(url);
                            }
                            //SendTeleV2(resObj.message.chat.id.ToString(), "Transaction success");
                            return;
