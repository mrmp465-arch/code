...
namespace Libs.Utils
{
    public static class LogCache
    {
        ...
        private static readonly object _lockGBank = new object();
        public static void AddPBankInfo(PBankInfo log)
        {
            //string keyCache = "LogPBankInfo";

            //lock (_lockGBank)
            //{
            //    var result = DataCaching.GetCache<List<PBankInfo>>(keyCache)
            //                 ?? new List<PBankInfo>();

            //    result = result.Where(x => x.RefCode != log.RefCode).ToList();
            //    result.Add(log);

            //    DataCaching.SetCache(keyCache, result, 14400/4); // 4 hours
            //}
        }
        public static void RemovePBankInfo(string log)
        {
            //string keyCache = "LogPBankInfo";
            ...
            //    var result = DataCaching.GetCache<List<PBankInfo>>(keyCache)
            //                 ?? new List<PBankInfo>();
            //    result = result.Where(x => x.RefCode != log).ToList();
            //   // result.Add(log);

            //    DataCaching.SetCache(keyCache, result, 14400); // 4 hours
            //}
        }
        public static List<PBankInfo> GetPBankInfo()
        {
            string keyCache = "LogPBankInfo";
