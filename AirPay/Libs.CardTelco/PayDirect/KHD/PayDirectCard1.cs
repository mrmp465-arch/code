...
{
    public class PayDirectCard1 : ICardTelcoHandler
    {

        protected string providerName = "paydirect1";
        ...
        protected string PartnerCode = "XBOM11";
        ...
        public APIResponse UseCard(APITransaction transaction)
        {
            ...
            try
            {
                ...
                cardRequest.signature =
                    Encrypts.MD5(string.Format("{0}{1}{2}{3}{4}{5}", cardRequest.issuer, cardRequest.cardCode,
                        cardRequest.transRef, cardRequest.partnerCode, cardRequest.password, SecretKey));

                NLogLogger.Info(new string[]
                {
                    "PayDirectCard", transaction.TransactionID.ToString(), "PayDirectCardRequest",
                    serializer.Serialize(cardRequest)
                });
                CardResult cardResult = ProcessCard(cardRequest);
                NLogLogger.Info(new string[]
                {
                    "PayDirectCard", transaction.TransactionID.ToString(), "PayDirectCardResponse",
            ...
            catch (Exception ex)
            {
                switch (step)
                {
                    case 1:
                        NLogLogger.Info(new string[]
                        {
                            "PayDirectCard", transaction.TransactionID.ToString(), "Error", "Step1",
                            ex.Message.Replace("\n", " ")
                        });
                    ...
                    case 2:
                        NLogLogger.Info(new string[]
                        {
                            "PayDirectCard", transaction.TransactionID.ToString(), "Error", "Step2",
                            ex.Message.Replace("\n", " ")
                        });
                        _APIResponse = new APIResponse((int) ResponseCode.SystemError);
                        break;
                    case 3:
                        NLogLogger.Info(new string[]
                        {
                            "PayDirectCard", transaction.TransactionID.ToString(), "Error", "UseCard",
                            ex.Message.Replace("\n", " ")
                        });
                    ...
                    case 4:
                        NLogLogger.Info(new string[]
                        {
                            "PayDirectCard", transaction.TransactionID.ToString(), "Error", "Step4",
                            ex.Message.Replace("\n", " ")
                        });
